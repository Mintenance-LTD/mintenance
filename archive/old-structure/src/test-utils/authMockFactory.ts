...
export class AuthMockFactory {
  static createCompleteAuthMock(overrides: Partial<AuthContextType> = {}): AuthContextType {
    return {
      ...
      signUp: createStub(),
      signOut: createStub(),
      updateProfile: createStub(),
      signInWithBiometrics: createStub(),
      isBiometricAvailable: global.jest?.fn?.(() => Promise.resolve(false)) || (() => Promise.resolve(false)),
      isBiometricEnabled: global.jest?.fn?.(() => Promise.resolve(false)) || (() => Promise.resolve(false)),
      enableBiometric: createStub(),
      disableBiometric: createStub(),
      ...overrides,
    };
  }
