...
export class AIPricingEngine {
  ...
  async analyzePricing(input: JobPricingInput): Promise<PricingAnalysis> {
    try {
      ...
      const mlPricingResult = await MLEngine.getPricingInsights({
        ...
        budget: input.homeownerBudget || 5000,
      ...
      const enhancedPricing = this.combineMLWithMarketData(
        mlPricingResult,
        marketData,
        input
      );
      ...
      const factors = this.generateEnhancedPricingFactors(
        input,
        complexity,
        marketData,
        mlPricingResult
      ...
      const recommendations = this.generateMLEnhancedRecommendations(
        input,
        enhancedPricing,
        factors,
        mlPricingResult
   ...
   */
  private combineMLWithMarketData(
    mlResult: any,
    marketData: MarketContext,
   ...
   */
  private generateEnhancedPricingFactors(
    input: JobPricingInput,
   ...
   */
  private generateMLEnhancedRecommendations(
    input: JobPricingInput,
    pricing: any,
    factors: PricingFactor[],
    mlResult: any
    ...
    if (mlResult.confidence < 70) {
      recommendations.push(
        'Consider requesting additional job details to improve pricing accuracy'
      );
    }

    if (mlResult.confidence > 85) {
      recommendations.push(
        'High confidence pricing - suitable for immediate quoting'
      );
    }
    ...
    if (mlResult.factors.complexity > 0.7) {
      recommendations.push(
        'Complex job detected - consider adding 10-20% contingency buffer'
      );
    }

    if (mlResult.factors.demand > 0.8) {
      recommendations.push(
        'High demand period - pricing can be at premium end of range'
      );
    }

    if (mlResult.factors.demand < 0.3) {
      recommendations.push(
        'Low demand period - consider competitive pricing to win job'
      );
    }
  ...
  private calculateBasePrice(
    ...
    const skillMultiplier =
      1 + (complexity.skillRequirements.length - 1) * 0.15;
    const riskMultiplier = 1 + complexity.riskLevel * 0.3;

    return (
      baseRate *
      complexity.timeEstimate *
  ...
  private generatePricingFactors(
    input: JobPricingInput,
    complexity: JobComplexityMetrics,
    market: MarketContext
  ): PricingFactor[] {
  ...
  private generateRecommendations(
    input: JobPricingInput,
    pricing: { min: number; max: number; optimal: number },
    factors: PricingFactor[]
  ): string[] {
    const recommendations: string[] = [];
    ...
    if (pricing.optimal > pricing.min * 1.3) {
      recommendations.push(
        'Consider starting at the lower end to attract more bids'
      );
    } else {
    ...
    if (complexityFactor) {
      recommendations.push(
        'Consider requiring photos/videos for accurate estimates'
      );
    }
    ...
    if (input.homeownerBudget && input.homeownerBudget < pricing.min) {
      recommendations.push(
        `Budget may be low - market rate is £${pricing.min}-£${pricing.max}`
      );
    } else if (input.homeownerBudget && input.homeownerBudget > pricing.max) {
      recommendations.push(
        'Budget is generous - you may attract premium contractors'
      );
    }
