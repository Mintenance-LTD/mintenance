...
export interface MutationStateWrapperProps<TData = any, TVariables = any> {
  ...
  children: (mutate: (variables: TVariables) => void, result: TData | undefined) => React.ReactNode;
...
export const QueryStateWrapper = <TData,>({
  ...
  const renderContent = () => {
    ...
    if (query.isError && !query.data) {
      const isNetworkError = !isOnline || query.error?.message?.includes('fetch');
...
export const useQueryState = <TData,>(query: UseQueryResult<TData>) => {
  ...
  return React.useMemo(() => ({
    ...
    isEmpty: query.isSuccess && (!query.data || (Array.isArray(query.data) && query.data.length === 0)),
