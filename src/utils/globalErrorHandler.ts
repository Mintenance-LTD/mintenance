...
export const setupGlobalErrorHandler = () => {
  ...
  const originalHandler = require('react-native/Libraries/Core/ExceptionsManager').unstable_setGlobalHandler;
  if (originalHandler) {
    originalHandler((error: any, isFatal: boolean) => {
      ...
      import('@sentry/react-native').then(({ captureException }) => {
        ...
        logger.warn('Sentry not available for promise rejection:', { data: e });
      });
    });
  }
};
...
export const trackPerformance = (operation: string, duration: number) => {
  ...
  import('@sentry/react-native').then(({ addBreadcrumb }) => {
    addBreadcrumb({
    ...
    logger.warn('Sentry not available for performance tracking:', { data: e });
  });
};
