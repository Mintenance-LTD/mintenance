...
export const useAIPricing = () => {
  ...
  const analyzeJobPricing = useMutation({
    mutationFn: async (input: JobPricingInput) => {
      logger.info('Analyzing job pricing with AI', {
  ...
  const getPricingSuggestions = useCallback(
    async (input: JobPricingInput): Promise<PricingAnalysis> => {
      try {
  ...
  const formatPricing = useCallback((analysis: PricingAnalysis) => {
    return {
      ...
      complexityLabel:
        analysis.complexity.charAt(0).toUpperCase() +
        analysis.complexity.slice(1),
  ...
  const getContractorRecommendations = useCallback(
    (analysis: PricingAnalysis, contractorProfile?: any) => {
      ...
      if (
        ...
        contractorProfile.completedJobs < 10
      ) {
        recommendations.push(
          'Consider competitive pricing to build your reputation'
        );
      }
  ...
  const getHomeownerInsights = useCallback(
    (analysis: PricingAnalysis, userBudget?: number) => {
      const insights = [];
      ...
      if (userBudget && userBudget < analysis.suggestedPrice.min) {
        ...
        insights.push({
          type: 'success',
          message:
            'Your budget is generous and should attract quality contractors',
        });
      } else {
        insights.push({
          type: 'info',
      ...
      if (
        ...
        analysis.complexity === 'specialist'
      ) {
        insights.push({
          type: 'info',
          message:
            'This job requires specialized skills - expect premium pricing',
        });
      }
