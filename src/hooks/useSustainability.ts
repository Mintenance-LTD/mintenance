...
export const SUSTAINABILITY_KEYS = {
  ...
  esgScore: (contractorId: string) => ['sustainability', 'esg', contractorId] as const,
  jobAnalysis: (jobId: string) => ['sustainability', 'job', jobId] as const,
  materialAlternatives: (materials: string[]) => ['sustainability', 'materials', materials] as const,
  carbonFootprint: (jobDetails: any) => ['sustainability', 'carbon', jobDetails] as const,
  ranking: (location: string, category?: string) => ['sustainability', 'ranking', location, category] as const,
  progress: (contractorId: string, timeframe: string) => ['sustainability', 'progress', contractorId, timeframe] as const,
};

// Hook for contractor ESG score calculation and retrieval
export const useContractorESGScore = (contractorId: string, autoCalculate: boolean = false) => {
  ...
  const esgQuery = useQuery({
    ...
    queryFn: async () => {
      if (autoCalculate) {
        return await sustainabilityEngine.calculateContractorESGScore(contractorId);
      }
  ...
  const recalculateESG = useMutation({
    ...
    onSuccess: (newScore) => {
      queryClient.setQueryData(SUSTAINABILITY_KEYS.esgScore(contractorId), newScore);
      logger.info('ESG score recalculated', { contractorId, score: newScore.overall_score });
    },
    onError: (error) => {
...
export const useJobSustainabilityAnalysis = (
  ...
  return useQuery({
    queryKey: SUSTAINABILITY_KEYS.jobAnalysis(`${jobTitle}-${category}-${location}`),
    queryFn: async () => {
...
export const useSustainableMaterials = (originalMaterials: string[]) => {
  return useQuery({
    ...
    queryFn: async () => {
      return await sustainabilityEngine.getSustainableMaterialAlternatives(originalMaterials);
    },
...
export const useSustainabilityRanking = (location: string, category?: string) => {
  return useQuery({
    queryKey: SUSTAINABILITY_KEYS.ranking(location, category),
    queryFn: async () => {
      return await sustainabilityEngine.getContractorSustainabilityRanking(location, category);
    },
...
export const useSustainabilityProgress = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      return await sustainabilityEngine.trackSustainabilityProgress(contractorId, timeframe);
    },
    enabled: Boolean(contractorId),
...
export const useSustainabilityFormatters = () => {
  const formatESGScore = (score: ESGScore) => {
    ...
    return {
      ...
      certificationLabel: score.certification_level.charAt(0).toUpperCase() + score.certification_level.slice(1)
    };
  };
  ...
  const formatMaterialSwap = (swap: MaterialSwapSuggestion) => {
    const savingsColor = swap.carbon_reduction > 5 ? '#10B981' : 
                        swap.carbon_reduction > 2 ? '#F59E0B' : '#6B7280';
    
    const availabilityIcon = swap.availability === 'readily_available' ? '✅' : 
                            swap.availability === 'order_required' ? '📦' : '⏳';

    return {
      carbonSavings: formatCarbonFootprint(swap.carbon_reduction),
      savingsColor,
      costImpact: swap.cost_difference > 0 ? `+${swap.cost_difference}%` : `${swap.cost_difference}%`,
      costColor: swap.cost_difference > 20 ? '#EF4444' : 
                swap.cost_difference > 10 ? '#F59E0B' : '#10B981',
      availabilityIcon,
      availabilityText: swap.availability.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())
    };
  };
  ...
  const getSustainabilityInsights = (analysis: JobSustainabilityAnalysis) => {
    ...
    if (analysis.sustainability_score >= 80) {
      insights.push({
        ...
        message: 'Excellent sustainability score! This job has minimal environmental impact.',
        icon: '🌱'
      });
      ...
      insights.push({
        ...
        message: 'Consider sustainable alternatives to reduce environmental impact.',
        icon: '⚠️'
      });
    }
...
export const sustainabilityUtils = {
  isHighImpactJob: (analysis: JobSustainabilityAnalysis) => {
    return analysis.predicted_impact.carbon_footprint_kg > 30 ||
           analysis.predicted_impact.waste_generated_kg > 20;
  },

  isPremiumGreenContractor: (esgScore: ESGScore) => {
    return esgScore.overall_score >= 80 && esgScore.certification_level === 'gold' || esgScore.certification_level === 'platinum';
  },
  ...
  calculateEnvironmentalROI: (
    ...
    return {
      ...
      paybackMonths: costIncrease > 0 ? Math.ceil((costIncrease / environmentalValue) * 12) : 0
    };
  },
  ...
  getComplianceStatus: (certificationLevel: string, location: string) => {
    ...
    const requirement = requirements[locationKey as keyof typeof requirements] || requirements.default;
    
    const scoreMap = { bronze: 50, silver: 65, gold: 80, platinum: 90 };
    const currentScore = scoreMap[certificationLevel as keyof typeof scoreMap] || 0;
