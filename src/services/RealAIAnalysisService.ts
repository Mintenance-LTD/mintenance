...
export class RealAIAnalysisService {
  ...
  private static async analyzeWithOpenAI(job: Job): Promise<AIAnalysis> {
    try {
      ...
      const messages: any[] = [
        ...
        {
          role: "user",
          content: job.photos && job.photos.length > 0 ? [
            { type: "text", text: userPrompt },
            ...job.photos.slice(0, 4).map(photo => ({ // Limit to 4 photos for API cost
              type: "image_url",
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: job.photos && job.photos.length > 0 ? "gpt-4-vision-preview" : "gpt-4-turbo-preview",
          messages,
          max_tokens: 800,
   ...
   */
  private static validateAndFormatOpenAIResponse(response: any, job: Job): AIAnalysis {
    return {
      confidence: Math.min(Math.max(response.confidence || 75, 0), 100),
      detectedItems: Array.isArray(response.detectedItems) ? response.detectedItems : ['AI analysis completed'],
      safetyConcerns: Array.isArray(response.safetyConcerns) ? response.safetyConcerns.map((c: any) => ({
        ...
        description: 'Follow appropriate safety protocols for this type of work'
      }],
      ...
      estimatedComplexity: ['Low', 'Medium', 'High'].includes(response.estimatedComplexity) 
        ...
        : 'Medium',
      suggestedTools: Array.isArray(response.suggestedTools) ? response.suggestedTools : ['Basic tools required'],
      estimatedDuration: response.estimatedDuration || '2-4 hours',
      detectedEquipment: Array.isArray(response.detectedEquipment) ? response.detectedEquipment.map((e: any) => ({
   ...
   */
  private static parseOpenAITextResponse(response: string, job: Job): AIAnalysis {
    // Extract information from text response using regex patterns
    const confidence = this.extractNumberFromText(response, /confidence[:\s]*(\d+)/i) || 85;
    
    const detectedItems = this.extractListFromText(response, /detected.{0,20}items?[:\s]*([^\n]*(?:\n(?!detected|safety|recommended|complexity|tools|duration)[^\n]*)*)/i) 
      || ['AI-analyzed maintenance issue'];
    
    const safetyConcerns = this.extractSafetyConcerns(response);
    const recommendedActions = this.extractListFromText(response, /recommended.{0,20}actions?[:\s]*([^\n]*(?:\n(?!detected|safety|recommended|complexity|tools|duration)[^\n]*)*)/i)
      || ['Follow standard maintenance procedures'];
    
    const complexity = this.extractComplexity(response);
    const suggestedTools = this.extractListFromText(response, /tools?[:\s]*([^\n]*(?:\n(?!detected|safety|recommended|complexity|tools|duration)[^\n]*)*)/i)
  ...
  private static extractNumberFromText(text: string, pattern: RegExp): number | null {
  ...
  private static extractSafetyConcerns(text: string): Array<{concern: string, severity: 'Low' | 'Medium' | 'High', description: string}> {
    const match = text.match(/safety.{0,20}concerns?[:\s]*([^\n]*(?:\n(?!detected|safety|recommended|complexity|tools|duration)[^\n]*)*)/i);
    if (!match) return [{
      concern: 'Standard safety precautions',
      severity: 'Medium',
      description: 'Follow appropriate safety protocols for this maintenance work'
    }];

    const concerns = match[1].split(/[,\n•\-\*]/).map(c => c.trim()).filter(c => c.length > 0);
  ...
  private static determineSeverity(concern: string): 'Low' | 'Medium' | 'High' {
    ...
    if (lowerConcern.includes('electrical') || lowerConcern.includes('fire') || lowerConcern.includes('toxic')) {
      return 'High';
    }
    if (lowerConcern.includes('injury') || lowerConcern.includes('damage') || lowerConcern.includes('leak')) {
      return 'Medium';
    }
    return 'Low';
  }
  ...
  private static extractComplexity(text: string): 'Low' | 'Medium' | 'High' {
    ...
    if (match) {
      ...
      return complexity.charAt(0).toUpperCase() + complexity.slice(1) as 'Low' | 'Medium' | 'High';
    }
    return 'Medium';
  }
  ...
  private static generateIntelligentFallback(job: Job): AIAnalysis {
    ...
    const analysisMap: { [key: string]: Partial<AIAnalysis> } = {
      plumbing: {
        detectedItems: [
          'Plumbing fixtures', 'Water supply lines', 'Drainage system', 'Pipe joints'
        ],
        safetyConcerns: [
          {
            concern: 'Water damage risk',
            severity: 'High' as const,
            description: 'Uncontrolled water can damage property and create mold conditions'
          },
          {
            concern: 'Slip hazard',
            severity: 'Medium' as const,
        ...
        estimatedComplexity: priority === 'high' ? 'High' as const : 'Medium' as const,
        suggestedTools: [
          'Adjustable pipe wrench set', 'Plumber\'s tape (Teflon)', 'Pipe cutter',
          'Drain snake/auger', 'Bucket and towels', 'Pipe thread compound'
        ],
      ...
      electrical: {
        detectedItems: [
          'Electrical panels', 'Wiring systems', 'Outlets and switches', 'Circuit breakers'
        ],
        safetyConcerns: [
          {
            concern: 'Electrocution hazard',
            severity: 'High' as const,
            description: 'Live electrical components pose serious injury or death risk'
          },
          {
            concern: 'Fire hazard',
            severity: 'High' as const,
        ...
        suggestedTools: [
          'Digital multimeter', 'Non-contact voltage tester', 'Wire strippers',
          'Electrical tape', 'Wire nuts/connectors', 'Insulated screwdrivers'
        ],
      ...
      hvac: {
        detectedItems: [
          'HVAC units', 'Ductwork systems', 'Air filtration', 'Thermostats'
        ],
        safetyConcerns: [
          {
          ...
          },
          {
            ...
            description: 'Inefficient systems increase utility costs significantly'
          }
        ],
        recommendedActions: [
        ...
        suggestedTools: [
          'HVAC gauges and manifolds', 'Coil cleaning supplies',
          'Replacement filters', 'Duct tape/sealant', 'Digital thermometer'
        ],
   ...
   */
  private static generateRealisticEquipment(job: Job): Array<{
    name: string;
    confidence: number;
    location: string;
  }> {
    ...
    const equipmentDatabase: { [key: string]: Array<{
      name: string;
      keywords: string[];
      locations: string[];
    ...
    const detectedItems: Array<{ name: string; confidence: number; location: string; }> = [];
    ...
    categoryEquipment.forEach(equipment => {
      ...
      if (hasKeyword || Math.random() > 0.5) { // 50% chance even without keywords
        const confidence = equipment.baseConfidence + (Math.random() * 10 - 5); // ±5% variation
        const location = equipment.locations[Math.floor(Math.random() * equipment.locations.length)];
  ...
  static isAIServiceConfigured(): boolean {
    return !!(this.OPENAI_API_KEY || this.AWS_ACCESS_KEY || this.GOOGLE_CLOUD_KEY);
  }
