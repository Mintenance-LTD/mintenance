...
export class AIAnalysisService {
  ...
  private static generateEnhancedAnalysis(job: Job): AIAnalysis {
    ...
    if (hasPhotos) {
      detectedItems = [...detectedItems, 'Visual damage assessment', 'Photo documentation'];
    }
  ...
  private static generateCategoryBasedAnalysis(job: Job): AIAnalysis {
    ...
    switch (category) {
      case 'plumbing':
        analysis = {
          confidence: 85,
          detectedItems: ['Plumbing fixtures', 'Water supply', 'Drainage system'],
          safetyConcerns: [
            {
          ...
          estimatedComplexity: description.includes('emergency') ? 'High' : 'Medium',
          suggestedTools: [
            'Pipe wrench set',
            'Plumber\'s tape',
            'Drain snake',
      ...
      case 'electrical':
        analysis = {
          confidence: 90,
          detectedItems: ['Electrical components', 'Wiring system', 'Circuit breakers'],
          safetyConcerns: [
            {
      ...
      case 'appliance':
        analysis = {
          confidence: 75,
          detectedItems: ['Appliance components', 'Electrical connections', 'Mechanical parts'],
          safetyConcerns: [
            {
   ...
   */
  private static generateEquipmentDetection(job: Job): Array<{
    name: string;
    confidence: number;
    location: string;
  }> {
    ...
    const equipmentSets: { [key: string]: Array<{ name: string; baseConfidence: number; locations: string[] }> } = {
      plumbing: [
        { name: 'Toilet', baseConfidence: 90, locations: ['Center', 'Left side', 'Right side'] },
        { name: 'Water Pipe', baseConfidence: 85, locations: ['Behind fixture', 'Under sink', 'Wall mounted'] },
        { name: 'Water Valve', baseConfidence: 80, locations: ['Lower right', 'Behind toilet', 'Under counter'] },
        { name: 'Drain', baseConfidence: 75, locations: ['Floor level', 'Sink area', 'Shower base'] }
      ],
      electrical: [
        { name: 'Electrical Panel', baseConfidence: 95, locations: ['Wall mounted', 'Basement', 'Utility room'] },
        { name: 'Wire Junction', baseConfidence: 80, locations: ['Inside wall', 'Ceiling area', 'Behind outlet'] },
        { name: 'Circuit Breaker', baseConfidence: 85, locations: ['Main panel', 'Sub panel', 'Distribution box'] },
        { name: 'Outlet', baseConfidence: 90, locations: ['Wall mounted', 'Counter level', 'Floor level'] }
      ],
      hvac: [
        { name: 'AC Unit', baseConfidence: 90, locations: ['Roof mounted', 'Ground level', 'Window mounted'] },
        { name: 'Air Duct', baseConfidence: 75, locations: ['Ceiling', 'Basement', 'Crawl space'] },
        { name: 'Thermostat', baseConfidence: 85, locations: ['Wall mounted', 'Central location', 'Hallway'] },
        { name: 'Air Filter', baseConfidence: 70, locations: ['Return vent', 'Unit interior', 'Duct system'] }
      ]
    };

    const equipment = equipmentSets[category || 'general'] || equipmentSets.general || [];
