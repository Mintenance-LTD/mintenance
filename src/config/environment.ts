  ...
  return {
    ...
    apiBaseUrl:
      ...
      (environment === 'production'
        ...
        : environment === 'staging'
          ? 'https://staging-api.mintenance.com'
          : 'http://localhost:3000'),
    ...
    // Feature Flags
    enableBiometricAuth: parseBoolean(
      process.env.EXPO_PUBLIC_ENABLE_BIOMETRIC_AUTH,
      true
    ),
    enablePushNotifications: parseBoolean(
    ...
    enableCrashReporting: parseBoolean(
    ...
    logLevel:
      (process.env.EXPO_PUBLIC_LOG_LEVEL as
        ...
        | 'info'
        | 'warn'
        | 'error') || (environment === 'development' ? 'debug' : 'info'),
    enableConsoleLogs: parseBoolean(
      process.env.EXPO_PUBLIC_ENABLE_CONSOLE_LOGS,
      environment === 'development'
    ),
...
export const validateConfig = (): { isValid: boolean; errors: string[] } => {
  ...
  if (config.environment === 'production') {
    if (!config.sentryDsn && config.enableCrashReporting) {
      errors.push(
        'EXPO_PUBLIC_SENTRY_DSN is required in production when crash reporting is enabled'
      );
    }

    if (!config.stripePublishableKey) {
      errors.push(
        'EXPO_PUBLIC_STRIPE_PUBLISHABLE_KEY is required in production'
      );
    }
