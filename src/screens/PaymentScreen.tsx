...
const PaymentScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const handlePayment = async () => {
    ...
    if (config.stripePublishableKey) {
      try {
        ...
        Alert.alert('Payment Error', error.message || 'Unable to initialize payment');
      } finally {
    ...
    try {
      ...
      Alert.alert('Demo Payment', 'Stripe is not configured. This is a demo flow only.', [
  ...
  const handleStripeSuccess = async (paymentIntentId: string) => {
    try {
      ...
      const escrow = await PaymentService.createEscrowTransaction(jobId, user.id, contractorId, amount);
      ...
      Alert.alert('Payment Error', error.message || 'Failed to finalize escrow payment');
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.footer}>
        {!clientSecret && (
          <Button
            variant="primary"
            title={initializingPayment || loading ? 'Processing...' : `Pay $${amount.toFixed(2)}`}
...
const styles = StyleSheet.create({
  ...
  amount: { fontSize: 24, fontWeight: 'bold', color: '#007AFF' },
  escrowInfo: { backgroundColor: '#E8F5E8', padding: 15, borderRadius: 8 },
  escrowTitle: { fontSize: 16, fontWeight: 'bold', color: '#2E7D32', marginBottom: 10 },
  escrowText: { fontSize: 14, color: '#2E7D32', marginBottom: 5 },
  paymentMethodContainer: { backgroundColor: theme.colors.surface, padding: 20, borderRadius: 12 },
  paymentMethodTitle: { fontSize: 18, fontWeight: 'bold', color: theme.colors.textPrimary, marginBottom: 15 },
  mockCardContainer: {
  ...
  mockCardText: { fontSize: 16, fontWeight: '600', color: theme.colors.textPrimary, marginBottom: 5 },
  mockCardSubtext: { fontSize: 14, color: theme.colors.textSecondary },
  mockNote: { fontSize: 12, color: theme.colors.textTertiary, fontStyle: 'italic' },
  footer: {
    padding: 20,
    backgroundColor: theme.colors.surface,
    borderTopWidth: 1,
