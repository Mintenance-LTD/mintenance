...
export class AuthService extends BaseService {
  ...
  async getCurrentUser(): Promise<User | null> {
    try {
      ...
      logger.error('Get current user error:', error, { service: 'general' });
      return null;
    }
  }
  async refreshToken(): Promise<AuthTokens | null> {
    try {
      ...
      logger.error('Refresh token error:', error, { service: 'general' });
      return null;
    }
  }
  async verifyEmail(token: string): Promise<boolean> {
    try {
      ...
      logger.error('Email verification error:', error, { service: 'general' });
      return false;
    }
  }
