/**
 * useBackgroundCheckGate — Tier 1 onboarding nudge to start the background
 * check. Gated on contractor + onboarded + identity submitted + check not
 * started + no dismissal. Mocks AsyncStorage, supabase (local maybeSingle),
 * useAuth, logger.
 */
...
import { useBackgroundCheckGate } from '../useBackgroundCheckGate';
...
beforeEach(() => {
  ...
  mockMaybeSingle.mockResolvedValue({
    data: { verification_status: 'verified', background_check_status: null },
    error: null,
  });
});
...
describe('probe / shouldShow', () => {
  it('is hidden for a signed-out user', async () => {
    ...
    const { result } = renderHook(() => useBackgroundCheckGate());
  ...
  it('is shown when identity is submitted and the check has not started', async () => {
    const { result } = renderHook(() => useBackgroundCheckGate());
    await waitFor(() => expect(result.current.shouldShow).toBe(true));
  });

  it('treats not_requested status as unstarted (shown)', async () => {
    mockMaybeSingle.mockResolvedValue({
      data: {
  ...
  it('is hidden when identity has not been submitted', async () => {
    mockMaybeSingle.mockResolvedValue({
      data: { verification_status: 'none', background_check_status: null },
      error: null,
    });
    const { result } = renderHook(() => useBackgroundCheckGate());
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  it('is hidden when the check has already started', async () => {
    mockMaybeSingle.mockResolvedValue({
      data: {
...
describe('dismiss + refresh', () => {
  async function ready() {
    const hook = renderHook(() => useBackgroundCheckGate());
