/**
 * useServiceAreaGate — nudges an onboarded contractor with zero service areas.
 * Mocks AsyncStorage, supabase (count via .eq()), useAuth, logger.
 */
...
import { useServiceAreaGate } from '../useServiceAreaGate';
...
describe('probe / shouldShow', () => {
  it('is hidden for a signed-out user', async () => {
    ...
    const { result } = renderHook(() => useServiceAreaGate());
  ...
  it('is hidden when the count query errors', async () => {
  ...
  it('is shown when the contractor has no service areas', async () => {
    const { result } = renderHook(() => useServiceAreaGate());
    await waitFor(() => expect(result.current.shouldShow).toBe(true));
  });

  it('is hidden when service areas already exist', async () => {
...
describe('dismiss + refresh', () => {
  async function ready() {
    const hook = renderHook(() => useServiceAreaGate());
