...
export const ImageCarousel: React.FC<ImageCarouselProps> = memo(({
  images, height = 300, width, showDots = true, gradientOverlay = false,
  overlayContent, onImagePress, style, testID,
}) => {
  ...
  if (images.length === 1) {
    return (
      <View style={[{ width: carouselWidth, height }, style]} testID={testID}>
        <TouchableOpacity activeOpacity={onImagePress ? 0.9 : 1} onPress={() => onImagePress?.(0)} disabled={!onImagePress} style={{ width: carouselWidth, height }}>
          <OptimizedImage source={{ uri: images[0] }} style={{ width: carouselWidth, height }} contentFit="cover" priority="high" cachePolicy="memory-disk" />
        </TouchableOpacity>
        {gradientOverlay && <LinearGradient colors={['transparent', 'rgba(0,0,0,0.4)']} style={StyleSheet.absoluteFill} pointerEvents="none" />}
        {overlayContent && <View style={styles.overlayContainer}>{overlayContent}</View>}
      </View>
    );
  }
  ...
  return (
    <View style={[{ width: carouselWidth, height }, style]} testID={testID}>
      <FlatList
        data={images} renderItem={renderImage} keyExtractor={(_, i) => `carousel-${i}`}
