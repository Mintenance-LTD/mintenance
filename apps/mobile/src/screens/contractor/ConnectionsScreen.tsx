...
export const ConnectionsScreen: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return connections.filter((c) => c.name.toLowerCase().includes(q) || c.trade.toLowerCase().includes(q));
  }, [connections, search]);
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            ...
            <TouchableOpacity
              ...
              onPress={() => toggleMutation.mutate({ id: item.id, follow: !item.is_following })}
            >
