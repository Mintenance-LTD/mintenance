...
export const FavoritesScreen: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, contractor_id, profiles:profiles!contractor_id(id, first_name, last_name, trade, rating, review_count, profile_image_url)'
        )
        .eq('user_id', user.id);
      if (error) throw new Error(error.message);
      return (data || []).map(
        (r: Record<string, unknown>): FavoriteContractor => {
  ...
  const removeMutation = useMutation({
    mutationFn: async (contractorId: string) => {
      await mobileApiClient.delete(
        `/api/properties/favorites?property_id=${contractorId}`
      );
    },
    onSuccess: () => {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Remove',
          style: 'destructive',
          onPress: () => removeMutation.mutate(item.contractor_id),
        },
      ]
    );
  };
  ...
  if (error)
    return <ErrorView message='Failed to load favourites' onRetry={refetch} />;
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            subtitle='Save contractors you like to find them quickly later.'
          />
        }
        renderItem={({ item }) => (
