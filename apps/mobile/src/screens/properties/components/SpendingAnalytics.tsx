...
export const SpendingAnalytics: React.FC<Props> = ({ jobs }) => {
  const completed = jobs.filter(j => j.status === 'completed' && j.budget > 0);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.totalBadge}>
          <Text style={styles.totalText}>{'\u00A3'}{totalSpent.toLocaleString('en-GB')}</Text>
        </View>
      </View>
      ...
      <View style={styles.chartRow}>
        {months.map((m, i) => (
          <View key={i} style={styles.barCol}>
            <View style={styles.barTrack}>
              <View
                style={[
                  styles.barFill,
                  {
                    height: m.amount > 0 ? `${Math.max((m.amount / maxAmount) * 100, 8)}%` : 0,
                    backgroundColor: m.amount > 0 ? theme.colors.primary : 'transparent',
                  },
                ]}
              />
            </View>
            ...
            {m.amount > 0 && (
              <Text style={styles.barAmount}>{'\u00A3'}{m.amount >= 1000 ? `${(m.amount / 1000).toFixed(1)}k` : m.amount}</Text>
            )}
          </View>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  totalBadge: {
    backgroundColor: theme.colors.primaryLight, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12,
  },
