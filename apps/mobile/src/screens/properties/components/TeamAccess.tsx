...
export const TeamAccess: React.FC<Props> = ({ propertyId }) => {
  ...
  const { data: members = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await mobileApiClient.get<{ members: TeamMember[] } | TeamMember[]>(
  ...
  const inviteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-team', propertyId] });
      setEmail('');
      setShowForm(false);
    },
        ...
        <View style={styles.form}>
          ...
          <View style={styles.roleRow}>
            {(['viewer', 'manager', 'admin'] as const).map(r => (
              <TouchableOpacity
                key={r}
                style={[styles.roleChip, role === r && { backgroundColor: ROLE_COLORS[r] }]}
                onPress={() => setRole(r)}
              >
          ...
          <TouchableOpacity style={styles.createBtn} onPress={handleInvite} disabled={inviteMutation.isPending}>
            <Text style={styles.createBtnText}>{inviteMutation.isPending ? 'Inviting...' : 'Invite'}</Text>
          </TouchableOpacity>
        </View>
      )}
          ...
          <View key={m.id} style={styles.memberRow}>
            ...
            <View style={styles.memberInfo}>
              ...
              <View style={styles.metaRow}>
                <View style={[styles.roleBadge, { backgroundColor: (ROLE_COLORS[m.role] || '#999') + '20' }]}>
                  <Text style={[styles.roleBadgeText, { color: ROLE_COLORS[m.role] || '#999' }]}>{m.role}</Text>
                </View>
                {m.status === 'pending' && (
...
const styles = StyleSheet.create({
  ...
  memberInfo: { flex: 1 },
  memberEmail: { fontSize: 14, fontWeight: '600', color: theme.colors.textPrimary },
  metaRow: { flexDirection: 'row', gap: 6, marginTop: 4 },
  roleBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  roleBadgeText: { fontSize: 11, fontWeight: '600' },
  pendingBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8, backgroundColor: '#FEF3C7' },
