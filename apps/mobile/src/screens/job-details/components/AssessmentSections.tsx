...
export function DamageSection({ damageType, severity, description, detectedItems }: DamageSectionProps) {
    ...
    <View style={s.section}>
      ...
      <View style={s.row}>
        <View style={[s.tag, { backgroundColor: '#EEF2FF' }]}>
          <Text style={[s.tagText, { color: '#4F46E5' }]}>{damageType.replace(/_/g, ' ')}</Text>
        </View>
...
export function HomeownerSection({ whatIsIt, whatToDo }: HomeownerSectionProps) {
  return (
    <View style={[s.section, s.blueCallout]}>
      <View style={s.row}>
        <Ionicons name="information-circle" size={18} color="#2563EB" />
        <Text style={[s.sectionTitle, { color: '#1E40AF', marginBottom: 0 }]}>What this means</Text>
      </View>
      <Text style={[s.bodyText, { color: '#1E3A5F' }]}>{whatIsIt}</Text>
      {whatToDo ? (
        <Text style={[s.bodyText, { color: '#1D4ED8', fontWeight: '600' }]}>Action: {whatToDo}</Text>
      ) : null}
    </View>
  );
}
  ...
  return (
    <View style={[s.section, s.redCallout]}>
      <View style={s.row}>
        ...
        <Text style={[s.sectionTitle, { color: '#991B1B', marginBottom: 0 }]}>Safety Hazards</Text>
      </View>
      <Text style={[s.bodyText, { color: '#7F1D1D' }]}>Safety Score: {safetyScore}/100</Text>
      {hazards.map((h, i) => (
        <View key={i} style={s.bulletRow}>
          <View style={s.bulletDot} />
          <Text style={[s.bodyText, { color: '#991B1B', flex: 1 }]}>
            <Text style={{ fontWeight: '700' }}>{h.type}: </Text>{h.description}
          </Text>
        </View>
      ))}
    </View>
...
export function CostSection({ min, recommended, max, estimatedTime, materials }: CostSectionProps) {
  const pct = max > min ? ((recommended - min) / (max - min)) * 100 : 50;
  return (
    <View style={s.section}>
      <Text style={s.sectionTitle}>Cost Estimate</Text>
      <Text style={s.costMain}>{'\u00A3'}{recommended.toLocaleString()}</Text>
      <View style={s.barTrack}>
        <View style={[s.barFill, { width: `${Math.min(100, pct)}%` }]} />
      </View>
      <View style={s.costRange}>
        <Text style={s.costLabel}>{'\u00A3'}{min.toLocaleString()}</Text>
        <Text style={s.costLabel}>{'\u00A3'}{max.toLocaleString()}</Text>
      </View>
      {estimatedTime ? <Text style={s.metaText}>Est. time: {estimatedTime}</Text> : null}
      {materials && materials.length > 0 && (
        <View style={{ marginTop: 8 }}>
          <Text style={s.subLabel}>Materials</Text>
          {materials.map((m, i) => (
            <View key={i} style={s.materialRow}>
              <Text style={s.bodyText}>{m.name} ({m.quantity})</Text>
              <Text style={[s.bodyText, { fontWeight: '600' }]}>{'\u00A3'}{m.estimatedCost}</Text>
            </View>
          ))}
        </View>
      )}
...
export function InsuranceSection({ riskScore, premiumImpact, mitigationSuggestions }: InsuranceSectionProps) {
  ...
  return (
    <View style={s.section}>
      ...
      <View style={[s.tag, { backgroundColor: impactStyle.bg, alignSelf: 'flex-start' }]}>
        <Text style={[s.tagText, { color: impactStyle.text }]}>{premiumImpact} impact</Text>
      </View>
...
export function UrgencyFooter({ urgency, timeline }: UrgencyFooterProps) {
  ...
  return (
    <View style={[s.urgencyBar, { backgroundColor: u.bg }]}>
      <Ionicons name={u.icon as keyof typeof Ionicons.glyphMap} size={16} color={u.text} />
...
const s = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, fontWeight: '700', color: theme.colors.textTertiary, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 },
  ...
  chip: { backgroundColor: theme.colors.backgroundSecondary, borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  bulletDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#DC2626', marginTop: 7 },
  costMain: { fontSize: 24, fontWeight: '700', color: '#2563EB', marginBottom: 8 },
  ...
  costRange: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  costLabel: { fontSize: 12, color: theme.colors.textTertiary },
  metaText: { fontSize: 12, color: theme.colors.textTertiary, marginTop: 4 },
  subLabel: { fontSize: 12, fontWeight: '700', color: theme.colors.textTertiary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 },
  materialRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 2 },
  urgencyBar: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 12, padding: 12, marginTop: 16 },
