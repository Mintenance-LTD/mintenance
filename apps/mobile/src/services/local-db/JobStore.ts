...
function mapRowToJob(row: DatabaseJobRow): Job {
  return {
    id: row.id,
    title: row.title,
...
export async function saveJob(
  ...
  const homeownerId =
    job.homeowner_id ||
    ((job as unknown as Record<string, unknown>).homeownerId as string) ||
    null;
  const contractorId =
    job.contractor_id ??
...
export async function getJob(
  ...
  const result = await db.getFirstAsync<DatabaseJobRow>(
