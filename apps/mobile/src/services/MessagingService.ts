        ...
        .insert([
          {
            ...
            message_text: safeMessageText, // Use 'message_text' column (schema uses 'message_text')
  ...
  static async deleteMessage(messageId: string, userId: string): Promise<void> {
    try {
      ...
      const { error } = await supabase
        .from('messages')
        .update({
          message_text: '[Message deleted]', // Use 'message_text' column (schema uses 'message_text')
          message_type: 'text',
          attachment_url: null,
        })
        .eq('id', messageId)
  ...
  static async searchJobMessages(
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq('job_id', jobId)
        .ilike('message_text', `%${sanitizedSearchTerm}%`) // Use 'message_text' column (schema uses 'message_text')
        .order('created_at', { ascending: false })
        .limit(limit);
