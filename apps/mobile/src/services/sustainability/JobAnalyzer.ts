...
export class JobAnalyzer {
  ...
  async analyzeJobSustainability(
    ...
    try {
      ...
      const improvementSuggestions =
        await this.materialAdvisor.generateEcoRecommendations({
          jobDetails: {
            title: jobTitle,
            description: jobDescription,
            category,
      ...
      return {
        ...
        certification_eligible: sustainabilityScore >= 75,
      };
    } catch (error) {
  ...
  async predictJobEnvironmentalImpact(jobDetails: {
    title: string;
    description: string;
    category: string;
    location: string;
  ...
  assessJobComplexity(description: string): number {
    ...
    if (
      ['renovation', 'installation', 'replacement', 'rewiring', 'major'].some(
        (k) => desc.includes(k)
      )
    )
      return 1.5;
    if (
      ['touch-up', 'minor', 'quick', 'simple', 'small'].some((k) =>
        desc.includes(k)
      )
    )
      return 0.7;
  ...
  calculateJobSustainabilityScore(impact: SustainabilityMetrics): number {
    ...
    return Math.round(
      carbonScore * 0.4 +
        wasteScore * 0.25 +
        impact.renewable_energy_percentage * 0.2 +
        impact.recycled_materials_percentage * 0.15
    );
  }

  async findGreenContractorsForJob(
    _category: string,
    _location: string,
    minESGScore: number
  ): Promise<string[]> {
    try {
      const { data, error } = await supabase
        ...
        .gte('total_score', minESGScore)
        .order('total_score', { ascending: false })
        .limit(10);
      if (error || !data) return [];
      return data.map((item: { contractor_id: string }) => item.contractor_id);
    } catch {
      return [];
    }
  }
