...
export async function uploadProfilePhotoExample() {
  ...
  logger.info(
    `Saved ${stats.savedMB.toFixed(2)} MB (${stats.savedPercentage.toFixed(0)}%)`,
    { service: 'mobile' }
  );
...
export async function uploadPropertyAssessmentExample(assessmentId: string) {
  ...
  logger.info('Full image:', compressed.uri, { service: 'mobile' });
  logger.info('Thumbnail:', compressed.thumbnailUri, { service: 'mobile' });
...
export async function smartCompressionExample(imageUri: string) {
  ...
  if (needsCompression) {
    logger.info('Image needs compression', { service: 'mobile' });
    const compressed = await compressJobPhoto(imageUri);
    return compressed.uri;
  } else {
    logger.info('Image is already optimized', { service: 'mobile' });
    return imageUri;
  }
}
...
export async function generateThumbnailsExample(imageUri: string) {
  ...
  logger.info('Thumbnails:', thumbnails, { service: 'mobile' });
...
export async function estimateSizeExample(imageUri: string) {
  ...
  logger.info(
    `Estimated compressed size: ${(estimatedSize / 1024 / 1024).toFixed(2)} MB`,
    { service: 'mobile' }
  );
...
export async function completeUploadFlowExample(jobId: string) {
  ...
  if (!permissions.camera) {
    logger.error('Camera permission required', { service: 'mobile' });
    return;
  }
...
export async function errorHandlingExample(imageUri: string) {
  try {
    ...
    if (result.error) {
      ...
      logger.error('Compression failed:', result.error, { service: 'mobile' });

      // Fallback: use original image
      logger.info('Using original image as fallback', { service: 'mobile' });
      return imageUri;
    }
