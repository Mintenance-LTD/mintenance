...
export async function compress(
  ...
  try {
    ...
    const compressedImage = await ImageManipulator.manipulateAsync(
      imageUri,
      manipulateActions,
      { compress: preset.quality, format: preset.format }
    );
  ...
  try {
    ...
    for (let i = 0; i < imageUris.length; i++) {
      ...
      if (i > 0 && results[i - 1] && !results[i - 1]!.error)
        await cleanupTempFile(results[i - 1]!.uri);
    }
...
export function getCompressionStats(result: CompressionResult): {
  ...
  const savedPercentage =
    result.originalSize > 0 ? (savedBytes / result.originalSize) * 100 : 0;
