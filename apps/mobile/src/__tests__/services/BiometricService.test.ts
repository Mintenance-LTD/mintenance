...
describe('BiometricService', () => {
  ...
  describe('enableBiometric', () => {
    ...
    it('throws error when biometric is not available', async () => {
      ...
      await expect(
        BiometricService.enableBiometric('test@example.com', {
          accessToken: 'access123',
          refreshToken: 'refresh123',
        })
      ).rejects.toThrow(
    ...
    it('throws error when session tokens are missing', async () => {
      await expect(
        BiometricService.enableBiometric('test@example.com', {
          accessToken: '',
          refreshToken: '',
        })
      ).rejects.toThrow('Session tokens are required to enable biometric authentication');
    });
  ...
  describe('authenticate', () => {
    ...
    it('throws error when stored credentials lack tokens', async () => {
      mockLocalAuth.authenticateAsync.mockResolvedValue({
        success: true,
        error: undefined,
        warning: undefined,
  ...
  describe('promptEnableBiometric', () => {
    ...
    it('shows prompt when biometric is available', async () => {
      const mockOnEnable = jest.fn(() => Promise.resolve());
      ...
      expect(mockOnEnable).toHaveBeenCalledWith('test@example.com');
    });
