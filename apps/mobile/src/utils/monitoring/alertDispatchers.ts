...
async function sendEmailAlert(alert: Alert): Promise<void> {
  logger.debug('MonitoringAndAlerting', 'Email alert (placeholder)', {
    alert: alert.id,
  });
}

async function sendPushAlert(alert: Alert): Promise<void> {
  logger.debug('MonitoringAndAlerting', 'Push alert (placeholder)', {
    alert: alert.id,
  });
}
    ...
    try {
      switch (channel) {
        ...
        case 'sentry':
          ...
          break;
        case 'webhook':
          await sendWebhookAlert(alert, webhookEndpoints, health);
          break;
        case 'email':
...
function summariseHealth(health: SystemHealth): Record<string, unknown> {
