...
export function createMockFactory<T extends Record<string, any>>(defaults: {
  [K in keyof T]: () => T[K];
}): MockFactory<T> {
  let state: Partial<T> = {};
...
export class MockDataGenerator {
  ...
  static generateJobs(count: number): unknown[] {
    return Array.from({ length: count }, (_, i) => {
      ...
      const category =
        this.jobCategories[
          Math.floor(Math.random() * this.jobCategories.length)
        ];
      ...
      return {
        ...
        location: {
          ...
          latitude: (location?.lat ?? 0) + (Math.random() - 0.5) * 0.1,
          longitude: (location?.lng ?? 0) + (Math.random() - 0.5) * 0.1,
          city: location?.city,
        ...
        contractorId:
          Math.random() > 0.7
            ? null
            : `contractor_${Math.floor(Math.random() * 5) + 1}`,
  ...
  static generateBids(
    ...
    if (jobs.length === 0 || contractors.length === 0) {
      ...
      if (contractors.length === 0 && jobs.length > 0) {
        contractors = (
          this.generateUsers(Math.max(3, count)) as Record<string, unknown>[]
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        ...
        amount: Math.floor(
          (((job as Record<string, unknown>).budget as number) ?? 100) *
            (0.8 + Math.random() * 0.4)
        ), // 80% - 120% of job budget
  ...
  static generateMessages(
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      const job =
        jobs.length > 0 ? jobs[Math.floor(Math.random() * jobs.length)]! : null;
      ...
      return {
        ...
        attachmentUrl:
          Math.random() > 0.8
            ? `https://picsum.photos/200/200?random=${i + 10}`
            : null,
  ...
  static generateDeviceInfo(): any {
    ...
    return {
      ...
      model:
        platform === 'ios'
          ? ['iPhone 12', 'iPhone 13', 'iPhone 14', 'iPad Pro'][
              Math.floor(Math.random() * 4)
            ]
          : ['Samsung Galaxy S21', 'Google Pixel 6', 'OnePlus 9'][
              Math.floor(Math.random() * 3)
            ],
      isDevice: true,
      manufacturer:
        platform === 'ios'
          ? 'Apple'
          : ['Samsung', 'Google', 'OnePlus'][Math.floor(Math.random() * 3)],
      screenWidth:
        screenDimensions.width + Math.floor(Math.random() * 100) - 50, // Add some variation
      screenHeight:
        screenDimensions.height + Math.floor(Math.random() * 100) - 50,
    };
  }
}
