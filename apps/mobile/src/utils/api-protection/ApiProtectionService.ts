...
export class ApiProtectionService {
  ...
  constructor(config: Partial<SecurityConfig> = {}) {
    ...
    this.rateLimitGuard = new RateLimitGuard((v) =>
      this.recordSecurityViolation(v)
    );
  ...
  async checkRequest(request: ApiRequest): Promise<{
    ...
    try {
      ...
      if (
        this.config.enableRequestValidation &&
        this.requestValidator.isBlockedUserAgent(request.userAgent)
      ) {
        this.recordSecurityViolation({
          type: 'blocked_agent',
          severity: 'medium',
      ...
      if (this.config.enableRateLimiting) {
        const rl = await this.rateLimitGuard.checkLimits(
          request,
      ...
      if (this.config.enableAbuseDetection) {
        const ab = await this.abuseDetector.check(
          request,
          this.getRequestIdentifier(request)
        );
  ...
  private recordSecurityViolation(violation: SecurityViolation): void {
    ...
    logger.warn('ApiProtectionService', 'Security violation', {
      type: violation.type,
  ...
  private sendSecurityAlert(pattern: AbusePattern, request: ApiRequest): void {
    logger.error('ApiProtectionService', 'Security alert', {
  ...
  private getRequestIdentifier(request: ApiRequest): string {
    return (
      request.userId || request.clientId || request.ipAddress || 'anonymous'
    );
  }
  ...
  getSecurityStats(): {
    ...
    const recentViolations = this.securityViolations.filter(
      (v) => v.timestamp > Date.now() - 86400000
    ).length;
    return {
      ...
      recentViolations,
      rateLimiterStats: this.rateLimitGuard.getStats(),
    };
  }
