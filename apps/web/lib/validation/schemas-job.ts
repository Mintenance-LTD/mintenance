...
// Deliberately NOT `.strict()`: this validates query-string params, and
// query strings legitimately carry extraneous keys (utm_*, fbclid, …) that
// must be ignored, not 400'd.
...
export const fileUploadSchema = z
  .object({
    ...
    fileSize: z
      ...
      .max(10485760, 'File size exceeds 10MB limit'),
    folder: z
      .string()
...
export const matchCommunicationSchema = z
  .object({
    contractorId: z.string().uuid('Invalid contractor ID'),
  })
  .strict();

export const enableLocationSharingSchema = z
  .object({
...
export type EnableLocationSharingInput = z.infer<
  typeof enableLocationSharingSchema
>;
