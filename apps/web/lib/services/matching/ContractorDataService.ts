...
export class ContractorDataService {
   ...
   */
  static async getAvailableContractors(
    ...
    try {
      ...
      if (error) {
        logger.error('Failed to get contractors', {
          error,
          service: 'matching',
        });
        return [];
      ...
      return contractorsWithDetails.filter(
        (c): c is ContractorProfile =>
          c !== null &&
          (criteria.location.maxDistance === 0 ||
            (c.distance || 0) <= criteria.location.maxDistance)
      );
    } catch (error) {
  ...
  static async assessAvailability(
    ...
    try {
      ...
      logger.error('Failed to assess availability', {
        error,
        contractorId,
        service: 'matching',
      });
