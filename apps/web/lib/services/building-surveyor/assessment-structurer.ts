...
export async function structureAssessment(
  ...
  evidence?: {
    ...
    sam3Segmentation?:
      ...
      | import('./types').SAM3SegmentationData
      | null
      | undefined;
    sceneGraphFeatures?:
  ...
  const normalizedMaterials: Material[] = (
    aiResponse.contractorAdvice?.materials || []
  ).map((material) => ({
  ...
  // Canonicalize damage type from free-text to 15 standard categories
  const rawDamageType = aiResponse.damageType || 'unknown_damage';
  const canonicalType = canonicalizeDamageType(rawDamageType);

  // Derive contractor trades from canonical type (AI may also provide them)
  const aiTrades = (aiResponse.contractorAdvice?.recommendedTrades ||
    []) as ContractorTrade[];
  const derivedTrades = DAMAGE_TYPE_TO_TRADES[canonicalType] || [
    'general_builder',
  ];
  // Merge: AI trades first, then fill from derived, dedup
  const recommendedTrades = [...new Set([...aiTrades, ...derivedTrades])].slice(
    0,
    5
  ) as ContractorTrade[];
  ...
  const contractorAdvice = {
    ...
    complexity:
      (aiResponse.contractorAdvice?.complexity as
        | 'low'
        | 'medium'
        | 'high'
        | undefined) || 'medium',
  ...
  const evidencePayload =
    evidence &&
    (evidence.roboflowDetections?.length ||
      evidence.visionAnalysis ||
      evidence.sam3Segmentation ||
      evidence.sceneGraphFeatures)
      ? {
  ...
  if (aiResponse.ricsConditionRating) {
    assessment.ricsConditionRating =
      aiResponse.ricsConditionRating as RICSConditionRating;
  }

  // Specialist Referrals (present only when AI identifies need)
  if (
    aiResponse.specialistReferrals &&
    aiResponse.specialistReferrals.length > 0
  ) {
    assessment.specialistReferrals =
