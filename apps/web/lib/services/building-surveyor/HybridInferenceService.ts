...
export class HybridInferenceService {
        ...
        try {
            ...
            if (result.internalPrediction && result.routingDecisionId) {
                const gpt4Agreement = result.route === 'hybrid' && result.agreementScore
                    ? result.agreementScore > 80 // Consider 80%+ agreement as "agreement"
                    : undefined;
                ...
                await ModelDriftDetectionService.recordPrediction(
                    ...
                    gpt4Agreement,
                    features // Pass image features for distribution analysis
                ).catch(error => {
    ...
    private static async selectRoute(
        ...
        // 2. Get internal prediction - use new ONNX inference if images available
    ...
    private static async executeInternalRoute(
        ...
        // Use new ONNX inference when images are available
