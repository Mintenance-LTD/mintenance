...
export class HybridInferenceService {
  ...
  static async assessDamage(
    ...
    try {
      ...
      const routeDecision = await this.selectRoute(
        features,
        context,
        imageUrls
      );
      ...
      switch (routeDecision.route) {
        case 'internal':
          result = await executeInternalRoute(
            imageUrls,
            features,
            context,
            routeDecision
        ...
        case 'gpt4_vision':
          ...
          break;
        case 'hybrid':
          result = await executeHybridRoute(
            imageUrls,
            features,
            context,
            routeDecision
      ...
      if (result.internalPrediction && result.routingDecisionId) {
        const gpt4Agreement =
          result.route === 'hybrid' && result.agreementScore
  ...
  private static async selectRoute(
    ...
    const isSafetyCritical = domain.safetyCriticalClasses.includes(
      internal.damageType
    );
    const hasSuspectedHazard =
      isSafetyCritical ||
   ...
   */
  static async getRoutingStatistics(timeRange?: { start: Date; end: Date }) {
