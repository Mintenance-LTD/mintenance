...
export type { ModelEvaluationMetrics } from './model-evaluation/types';
export {
  quickEvaluate,
  validateModelFile,
} from './model-evaluation/standalone';
...
export class ModelEvaluationService {
  ...
  static async extractMetricsFromTraining(
    ...
    try {
      ...
      logger.error('Failed to extract training metrics', {
        error,
        modelVersion,
      });
      throw error;
  ...
  static async evaluateModel(
    ...
    try {
      ...
      const args = [
        ...
        modelPath,
        '--dataset',
        options.test_dataset_path || 'datasets/test',
        '--conf',
        String(options.confidence_threshold || 0.25),
        '--iou',
        String(options.iou_threshold || 0.45),
        '--max-det',
        String(options.max_detections || 300),
        '--device',
        options.device || 'cpu',
        '--batch',
        String(options.batch_size || 1),
        '--output-json',
        `evaluation-${modelVersion}.json`,
      ];
      ...
      const evaluationResults = JSON.parse(
        readFileSync(evaluationPath, 'utf-8')
      );
  ...
  static async compareModels(
    ...
    try {
      ...
      const avgImprovement =
        ...
        4;
      const isSignificant =
        Math.abs(avgImprovement) >
        this.DEPLOYMENT_THRESHOLDS.MIN_IMPROVEMENT_FOR_DEPLOYMENT;
  ...
  static async calculateMetricsTrend(
    ...
    try {
      ...
      const bestModel = historicalMetrics.reduce((best, current) =>
        current.test_metrics.f1_score > best.test_metrics.f1_score
          ? current
          : best
      );
      ...
      const f1Scores = historicalMetrics
        .map((m) => m.test_metrics.f1_score)
        .reverse();
