...
export class DriftMonitorService {
  ...
  /**
   * Detect drift and automatically adjust fusion weights
   * Used by the cron job for automated adaptation
   */
  static async detectAndAdjustWeights(): Promise<DriftDetectionResult> {
    try {
      // Get current context (could be enhanced with region detection)
      ...
      if (driftResult.hasDrift && driftResult.driftScore > this.DRIFT_THRESHOLD) {
        ...
        if (sum > 0) {
          ...
          adjustedWeights.sam /= sum;
        }

        // Store drift event in database for tracking
        await this.storeDriftEvent(driftResult, adjustedWeights);
      ...
      logger.error('Failed to detect and adjust weights for drift', { error });
      throw error;
    }
  }

  /**
   * Store drift event in database for tracking and analysis
   */
  private static async storeDriftEvent(
    driftResult: DriftDetectionResult,
    adjustedWeights: { yolo: number; maskrcnn: number; sam: number }
  ): Promise<void> {
    try {
      const supabase = serverSupabase;

      // Store in a drift_events table (would need migration)
      // For now, we'll log it
      logger.info('Drift event recorded', {
      ...
      //     original_weights: currentWeights,
      //     adjusted_weights: adjustedWeights,
      //     created_at: new Date().toISOString()
      //   });
    } catch (error) {
      logger.error('Failed to store drift event', { error });
