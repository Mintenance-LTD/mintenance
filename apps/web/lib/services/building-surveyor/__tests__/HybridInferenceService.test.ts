...
vi.mock('../config/BuildingSurveyorConfig', () => ({
  getConfig: vi.fn(() => ({
    ...
    abTest: {
      sfnRateThreshold: 0.1,
      coverageViolationThreshold: 5.0,
      automationSpikeThreshold: 20.0,
      criticObservationsThreshold: 100,
      calibrationDataThreshold: 100,
    },
...
const mockGpt4Assessment = {
  ...
  insuranceRisk: {
    riskFactors: [],
    riskScore: 20,
    premiumImpact: 'low' as const,
    mitigationSuggestions: [],
  },
  urgency: {
    urgency: 'monitor' as const,
    recommendedActionTimeline: 'Monitor for changes',
...
describe('HybridInferenceService', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(ModelDriftDetectionService.recordPrediction).mockResolvedValue(
      undefined
    );
    vi.mocked(AssessmentOrchestrator.assessDamage).mockResolvedValue(
      mockGpt4Assessment as any
    );
  ...
  describe('Confidence Thresholds', () => {
    ...
    it('should route correctly at threshold boundaries', async () => {
      ...
      let result = await HybridInferenceService.assessDamage(
        mockImageUrls,
        mockContext
      );
      expect(result.route).toBe('internal');
      ...
      result = await HybridInferenceService.assessDamage(
        mockImageUrls,
        mockContext
      );
      expect(result.route).toBe('hybrid');
  ...
  describe('Assessment Conversion', () => {
    it('should convert internal prediction to Phase1BuildingAssessment format', async () => {
      ...
      expect(result.assessment.damageAssessment.confidence).toBe(
    ...
    it('should set correct urgency timeline based on urgency level', async () => {
      ...
      const urgencyTests = [
        ...
        {
          urgency: 'monitor' as const,
          expectedTimeline: 'Monitor for changes',
        },
      ];
      ...
      for (const test of urgencyTests) {
        ...
        if (test.urgency === 'immediate') {
          ...
          expect(result.assessment.urgency.recommendedActionTimeline).toBe(
            test.expectedTimeline
          );
        }
      }
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle internal model prediction failures', async () => {
      vi.mocked(InternalDamageClassifier.isModelReady).mockResolvedValue(true);
      vi.mocked(InternalDamageClassifier.predictFromImage).mockRejectedValue(
        new Error('Model error')
      );

      // Should fallback to GPT-4 or throw
      await expect(
        HybridInferenceService.assessDamage(mockImageUrls, mockContext)
      ).rejects.toThrow();
    });
  });
