...
export async function extractHandcraftedFeatures(
  ...
  const visionHasMold =
    Boolean(
      visionSummary?.detectedFeatures?.some((feature) =>
        feature.toLowerCase().includes('mold')
      )
    ) ||
    Boolean(
      visionSummary?.labels?.some((label) =>
  ...
  const uniqueDetectionClasses =
    detectionCount > 0
      ? new Set((roboflowDetections || []).map((det) => det.className)).size
      : 0;
  const detectionConfidenceNormalized =
    detectionCount > 0
      ? Math.min(1.0, avgDetectionConfidence / 100)
      : visionConfidenceNormalized;
  const classDiversity =
    detectionCount > 0
      ? Math.min(1.0, uniqueDetectionClasses / detectionCount)
      : 0;
  const hazardSignal =
    detectionCount > 0
      ? Math.min(
          1.0,
          (moldDetections + crackDetections + moistureDetections) /
            Math.max(1, detectionCount)
        )
  ...
  const totalDetectionArea = detectionAreas.reduce(
    (sum, area) => sum + area,
    0
  );
  const largestDetectionArea = detectionAreas.reduce(
    (max, area) => Math.max(max, area),
    0
  );
  ...
  const averageDetectionAreaNormalized =
    detectionCount > 0
      ? Math.min(
          1.0,
          totalDetectionArea / detectionCount / config.imageBaseArea
        )
      : 0;
  const maxDetectionAreaNormalized =
    detectionCount > 0
      ? Math.min(1.0, largestDetectionArea / config.imageBaseArea)
      : 0;
  const moldRatio = detectionCount > 0 ? moldDetections / detectionCount : 0;
  const structuralRatio =
    detectionCount > 0 ? crackDetections / detectionCount : 0;
  ...
  if (assessment) {
    ...
    features.push(
      Math.min(1.0, assessment.compliance.complianceIssues.length / 10)
    );
 ...
 */
function encodeLocation(location: string): number {
 ...
 */
function encodeBuildingStyle(propertyDetails: string): number {
  ...
  if (details.includes('modern') || details.includes('contemporary'))
    return 0.8;
 ...
 */
function encodeDamageType(damageType: string): number {
 ...
 */
function encodeDamageLocation(location: string): number {
 ...
 */
function encodeUrgency(urgency: UrgencyLevel): number {
