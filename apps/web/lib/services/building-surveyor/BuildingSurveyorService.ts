...
export class BuildingSurveyorService {
  ...
  static async assessDamage(
    ...
    try {
      ...
      // Estimate cost and check budget before making API call
      const estimatedTokens = 1500 + (imagesToAnalyze.length * 500); // Rough estimate
      const estimatedCost = CostControlService.estimateCost('gpt-4o', {
        inputTokens: estimatedTokens,
        outputTokens: 2000, // max_tokens
      ...
      if (usage) {
        const actualCost = CostControlService.estimateCost('gpt-4o', {
          inputTokens: usage.prompt_tokens,
          outputTokens: usage.completion_tokens
        });

        await CostControlService.recordUsage('building-surveyor', 'gpt-4o', actualCost, {
      ...
      // 11. Shadow mode: Log decision but optionally allow actual decisions in testing
      ...
      if (shadowModeEnabled) {
        ...
        if (shadowModeTestingEnabled) {
          ...
          // Production shadow mode: Always force escalation
          finalDecision = 'escalate';
