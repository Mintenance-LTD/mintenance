 ...
 */
function sanitiseMLLabel(label: string, maxLength = 100): string {
  return (
    label
...
export function buildEvidenceSummary(
  ...
  if (roboflowDetections.length > 0) {
    const topClasses = roboflowDetections.reduce<
  ...
  if (visionAnalysis) {
    const topLabels = visionAnalysis.labels
      ...
      .map(
        (label) =>
          `${sanitiseMLLabel(label.description)} (${Math.round(label.score * 100)}%)`
      );
    const topObjects = visionAnalysis.objects
      .slice(0, 5)
      .map(
        (obj) =>
          `${sanitiseMLLabel(obj.name)} (${Math.round(obj.score * 100)}%)`
      );
    const features = visionAnalysis.detectedFeatures
      .slice(0, 6)
      .map((f) => sanitiseMLLabel(f))
      .join(', ');

    summaryParts.push(
      `Vision confidence ${Math.round(visionAnalysis.confidence)}%`
    );
