...
export async function executeGPT4Route(
  ...
  const assessment = await AssessmentOrchestrator.assessDamage(
    imageUrls,
    context
  );
...
function calculateAgreementScore(
  ...
  if (
    internal.damageType.toLowerCase() ===
    gpt4.damageAssessment.damageType.toLowerCase()
  ) {
    score += 0.4;
  }
 ...
 */
function convertInternalPredictionToAssessment(
  ...
  return {
    ...
    safetyHazards: {
      hazards: (prediction.safetyHazards ||
        []) as import('../types').SafetyHazard[],
...
function calculatePremiumImpact(
