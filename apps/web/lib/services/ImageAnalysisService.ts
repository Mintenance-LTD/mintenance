 ...
 * - LRU cache with O(1) operations (vs previous O(n log n) eviction)
...
export class ImageAnalysisService {
  ...
  private static cache: LRUCache<string, ImageAnalysisResult> = new LRUCache({
    max: ImageAnalysisService.MAX_CACHE_SIZE,
    ttl: ImageAnalysisService.CACHE_TTL,
    // Update LRU on get - frequently accessed items stay in cache
    updateAgeOnGet: true,
    // Update LRU on has - checking existence counts as access
   ...
   * Performance: O(n log n) for sort, but only done once per analysis
   */
   ...
   * Performance: O(1) lookup with LRU cache
   */
  ...
  static async analyzePropertyImages(
    ...
    try {
      ...
      // Analyze each image with timeout protection
      ...
      // Previous: Sequential processing (12s for 2 images, 24s for 4 images)
  ...
  static getCacheStats(): {
    ...
    return {
      ...
      // LRU cache doesn't track hit rate by default, but we could add it if needed
    };
  }

  /**
   * Prune expired entries manually (LRU cache does this automatically, but exposed for monitoring)
   */
  static pruneCache(): number {
    const sizeBefore = this.cache.size;
    this.cache.purgeStale();
    const sizeAfter = this.cache.size;
    const prunedCount = sizeBefore - sizeAfter;

    if (prunedCount > 0) {
      logger.info('Image analysis cache pruned', {
