...
export class AgentAnalytics {
  ...
  static async getLearningProgress(agentName: string): Promise<LearningData | null> {
    try {
      ...
      return {
        agentName,
        accuracyHistory: accuracyHistory as Array<{ date: string; accuracy: number }>,
        memoryUpdateFrequency: memoryUpdateFrequency as Array<{ date: string; updates: number }>,
        confidenceTrend: confidenceTrend as Array<{ date: string; confidence: number }>,
