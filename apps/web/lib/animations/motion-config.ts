...
function createMotionVariant(
  ...
  if (shouldReduce) {
    ...
    return {
      initial: config.animate as Variant,
      animate: config.animate as Variant,
      exit: config.animate as Variant,
    };
  }

  return {
    initial: config.initial as Variant,
    animate: config.animate as Variant,
    exit: (config.exit || config.initial) as Variant,
  };
}
