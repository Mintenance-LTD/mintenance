...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      ...
      retry: (
        failureCount: number,
        error: Error | { status?: number; name?: string; message?: string }
      ) => {
...
export const queryKeys = {
  ...
  contractors: {
    ...
    gallery: (contractorId: string) =>
      ['contractors', 'gallery', contractorId] as const,
  },
  ...
  payments: {
    ...
    transactions: (userId: string) =>
      ['payments', 'transactions', userId] as const,
  },
...
const queryUtils = {
  ...
  invalidatePayments: (userId: string) => {
    queryClient.invalidateQueries({
      queryKey: queryKeys.payments.intents(userId),
    });
    queryClient.invalidateQueries({
