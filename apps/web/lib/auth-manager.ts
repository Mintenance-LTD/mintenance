...
export class AuthManager {
  ...
  async login(
    ...
    try {
      ...
      logger.info('Attempting login with Supabase Auth', {
        email,
        service: 'auth',
      });
      ...
      if (!authData.user) {
        logger.error('No user returned from Supabase Auth login', undefined, {
          service: 'auth',
        });
        return {
          success: false,
  ...
  async register(userData: RegisterData): Promise<AuthResult> {
    try {
      logger.info('Starting user registration with Supabase Auth', {
      ...
      const passwordValidation = DatabaseManager.isValidPassword(
        userData.password
      );
      ...
      if (!authData.user) {
        logger.error('No user returned from Supabase Auth', undefined, {
          service: 'auth',
        });
        return {
          success: false,
      ...
      logger.info('Creating authentication cookie headers', {
        userId: user.id,
        service: 'auth',
      });
      const cookieHeaders = createAuthCookieHeaders(
      ...
      logger.info('Authentication cookie headers created', {
        userId: user.id,
        service: 'auth',
      });
  ...
  async changePassword(
    ...
    try {
      ...
      // Update password
      const success = await DatabaseManager.updateUserPassword(
        userId,
        newPassword
      );
