...
export class FeatureFlagService {
    ...
    try {
      ...
      if (!isDevelopment && ldKey) {
        ...
        try {
          ...
          logger.warn(
            'LaunchDarkly SDK not available, falling back to local flags'
          );
  ...
  shouldUseSAM3PresenceDetection(userId?: string): boolean {
    ...
    if (config.rolloutPercentage < 100) {
      ...
      const hash = this.hashUserId(
        userId || this.userContext.key || 'anonymous'
      );
  ...
  private async triggerRollback(reason: string): Promise<void> {
    if (!this.getFlag(FeatureFlag.AUTO_ROLLBACK, true)) {
      logger.warn('Auto rollback disabled, manual intervention required', {
        reason,
      });
      return;
    }
    ...
    try {
      ...
      this.flags[FeatureFlag.SAM3_PRESENCE_DETECTION] = {
        ...(DEFAULT_FLAGS[FeatureFlag.SAM3_PRESENCE_DETECTION] as Record<
          string,
          unknown
        >),
        enabled: false,
