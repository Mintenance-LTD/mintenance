...
export const PATCH = withApiHandler(
  ...
  async (request, { user, params }) => {
    ...
    const { data: existingComment, error: fetchError } = await serverSupabase
    ...
    if (is_solution !== undefined) {
      ...
      if (
        post &&
        (post.contractor_id === user.id ||
          existingComment.contractor_id === user.id)
      ) {
        updateData.is_solution = is_solution;
        if (is_solution) updateData.solution_verified_by = user.id;
      } else {
        throw new ForbiddenError(
          'Only post author or comment author can mark as solution'
        );
      }
    }
    ...
    const { data: updatedComment, error: updateError } = await serverSupabase
      ...
      .eq('id', commentId)
      .select(
        `*, contractor:contractor_id (id, first_name, last_name, profile_image_url)`
      )
      .single();
