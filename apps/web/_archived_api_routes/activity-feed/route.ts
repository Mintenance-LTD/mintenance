  ...
  async () => {
    ...
    for (const window of [oneHourAgo, oneDayAgo, sevenDaysAgo]) {
      const { data, error } = await serverSupabase
        .from('jobs')
        .select(
          'id, title, category, status, updated_at, city, homeowner_id, contractor_id'
        )
        .eq('status', 'assigned')
        .gte('updated_at', window.toISOString())
        .order('updated_at', { ascending: false })
    ...
    const jobMap = new Map(
      (bidJobs || []).map(
        (
          j: Pick<
            JobRecord,
            'id' | 'title' | 'category' | 'city' | 'homeowner_id'
          >
        ) => [j.id, j as JobRecord]
      )
    );
    ...
    jobsWithBids?.forEach((bid) => {
      quotesByJob.set(bid.job_id, (quotesByJob.get(bid.job_id) || 0) + 1);
    });
    ...
    completedJobs?.forEach((job) => {
      ...
      activities.push({
        name:
          firstName +
          (name.includes(' ') ? ' ' + name.split(' ')[1].charAt(0) + '.' : ''),
        action: `completed a ${job.category || 'project'}`,
        location: job.city || 'UK',
    ...
    hiredJobs?.forEach((job) => {
      ...
      activities.push({
        name:
          firstName +
          (name.includes(' ') ? ' ' + name.split(' ')[1].charAt(0) + '.' : ''),
        action: `hired a ${getContractorType(job.category || 'contractor')}`,
        location: job.city || 'UK',
        time: formatTimeAgo(new Date(job.updated_at)),
        timestamp: new Date(job.updated_at),
      });
    ...
    recentJobs24h?.forEach(
      (job: Pick<JobRecord, 'homeowner_id' | 'contractor_id'>) => {
