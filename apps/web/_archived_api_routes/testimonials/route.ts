...
export const GET = withApiHandler(
  ...
  async () => {
    ...
    const reviewerIds = [
      ...new Set(
        (reviews || []).map((r: ReviewRecord) => r.reviewer_id).filter(Boolean)
      ),
    ];
    ...
    if (reviewerIds.length > 0) {
      ...
      if (reviewerError) {
        ...
        reviewerMap = new Map(
          (reviewers || []).map((r: ReviewerRecord) => [r.id, r])
        );
      }
    }
    ...
    const testimonials = (reviews || []).map((review: ReviewRecord) => {
      const reviewer =
        reviewerMap.get(review.reviewer_id) || ({} as ReviewerRecord);
      const job = Array.isArray(review.jobs)
        ? review.jobs[0]
        : review.jobs || ({} as ReviewJob);
