...
// Logger utility (inline — service workers cannot use ES module imports)
const logger = {
...
/**
 * Handle chunk loading errors by clearing cache and notifying client
 */
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'CHUNK_LOAD_ERROR') {
    logger.warn('[SW] Chunk load error detected, clearing cache...');

    event.waitUntil(
      caches.keys().then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            logger.info('[SW] Deleting cache:', cacheName);
            return caches.delete(cacheName);
          })
        );
      }).then(() => {
        ...
        return self.clients.matchAll().then((clients) => {
          clients.forEach((client) => {
            client.postMessage({
              type: 'CACHE_CLEARED',
              message: 'Cache cleared due to chunk load error'
            });
          });
        });
      })
  ...
  if (event.data && event.data.type === 'SKIP_WAITING') {
    logger.info('[SW] Skip waiting requested');
    self.skipWaiting();
  }
});
