  ...
  withApiHandler:
    ...
    (request: NextRequest) =>
      handler(request, {
        user: {
          id: fixture.actor,
          role: fixture.actor === 'contractor' ? 'contractor' : 'homeowner',
        },
...
import { POST } from '@/app/api/contracts/[id]/accept/route';
const request = () =>
  POST(
    new NextRequest('http://localhost/api/contracts/test/accept', {
      method: 'POST',
...
describe('contract signing replay authorization', () => {
  beforeEach(() => {
  ...
  it.each(['owner', 'payer', 'contractor'])(
