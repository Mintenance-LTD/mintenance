...
// Idempotency wrapper (added 2026-05-27, AUDIT_PUNCH_LIST P2 #75) is
// infrastructure these tests don't assert on — pass through releaseOnError so
// the route's contract-signing logic runs unwrapped. Without this, the real
// module calls serverSupabase.rpc (unmocked) and fails closed.
...
describe('POST /api/contracts/[id]/accept', () => {
  ...
  it('should return 400 when contract ID is invalid UUID', async () => {
    ...
    const req = createPostRequest(
      'http://localhost:3000/api/contracts/not-a-uuid/accept'
    );
  ...
  it('should return 404 when contract does not exist', async () => {
    ...
    const req = createPostRequest(
      'http://localhost:3000/api/contracts/bad-id/accept'
    );
  ...
  it('should return 400 when contractor has already signed', async () => {
    setupContractMocks({
      contractData: {
        ...pendingContract,
        contractor_signed_at: '2026-01-01T00:00:00Z',
      },
    });
  ...
  it('should notify homeowner when contractor signs (pending)', async () => {
    ...
    // 2026-05-21 Mint Editorial voice: pending-signature copy was rewritten
    // from the generic "Contract Pending Your Signature" to a signer-named,
    // action-led title ("<signer> signed — your turn"). userId/type unchanged.
    expect(mocks.createNotification).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'homeowner-1',
        title: expect.stringContaining('your turn'),
  ...
  it('should notify both parties when contract becomes accepted', async () => {
    ...
    // 2026-05-21 Mint Editorial voice: the fully-accepted fan-out title was
    // rewritten from "Contract Accepted!" to "<job> — contract is live" and
    // now names the next escrow step in the body. userId/type unchanged.
    // Should notify contractor
    expect(mocks.createNotification).toHaveBeenCalledWith(
      expect.objectContaining({
        userId: 'contractor-1',
        title: expect.stringContaining('contract is live'),
