...
function createChainMock(terminals: Record<string, unknown> = {}) {
  ...
  chain.then = vi.fn((resolve: (v: unknown) => void) =>
    resolve(terminals.resolve ?? { data: [], error: null })
  );
...
function setupAcceptMocks(
  ...
  const jobResult = {
    data: overrides.jobData ?? postedJob,
  ...
  mocks.supabaseFrom.mockImplementation((table: string) => {
    ...
    if (table === 'message_threads') {
      ...
      chain.update = vi
        .fn()
        .mockReturnValue(createChainMock({ resolve: { error: null } }));
      return chain;
    }
    if (table === 'messages') {
...
describe('POST /api/jobs/[id]/bids/[bidId]/accept', () => {
  ...
  it('should return 404 when job does not exist', async () => {
    ...
    const req = createPostRequest(
      'http://localhost:3000/api/jobs/bad-id/bids/bid-1/accept'
    );
  ...
  it('should return 404 when bid does not exist', async () => {
    ...
    const req = createPostRequest(
      'http://localhost:3000/api/jobs/job-1/bids/bad-bid/accept'
    );
  ...
  it('should return 409 when bid update fails with constraint violation', async () => {
