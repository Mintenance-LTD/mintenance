...
vi.mock('@/lib/services/payment/RefundRecoveryService', () => ({
  runRefundRecovery: state.run,
}));
...
import { GET } from '@/app/api/cron/refund-recovery/route';
const secret = 'synthetic-cron-test-value';
const path = '/api/cron/refund-recovery';
const request = (headers: Record<string, string> = {}) =>
  new NextRequest(`http://localhost${path}`, { headers });
describe('refund recovery cron authentication', () => {
  beforeEach(() => {
    vi.stubEnv('CRON_SECRET', secret);
    vi.stubEnv('CRON_SECRET_REFUND_RECOVERY', '');
    vi.stubEnv('CRON_HMAC_TOLERANCE_SECONDS', '300');
    state.run
      .mockReset()
      .mockResolvedValue({ processed: 0, confirmed: 0, pending: 0, failed: 0 });
    const q = {
      insert: () => q,
      select: () => q,
