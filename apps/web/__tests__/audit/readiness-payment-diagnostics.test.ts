  ...
  self.then = (
    ...
    Promise.resolve(
      state.usedSingle
        ...
        : { data: overrides?.listData?.() ?? [], count: 0, error: null }
    ).then(resolve);
...
describe('create-intent header-less duplicate protection', () => {
  ...
  it('resumes the same pending payment after its escrow insert is visible', async () => {
    const first = await callRoute(makeHeaderlessRequest());
    expect(first.status).toBe(200);
    const created = mocks.stripePaymentIntentsCreate.mock.calls[0][0];
  ...
  it.each([
    ...
    { status: 'canceled' },
    { client_secret: null },
    { metadata: { payerId: 'other-payer' } },
    { metadata: { contractorId: 'other-contractor' } },
    { metadata: { jobId: 'other-job' } },
    { metadata: { bidId: 'other-bid' } },
    { metadata: { creditAppliedPence: '-1' } },
  ])(
    'does not resume mismatched or non-resumable payment %j',
    async (override) => {
      expect((await callRoute(makeHeaderlessRequest())).status).toBe(200);
      const created = mocks.stripePaymentIntentsCreate.mock.calls[0][0];
      mocks.stripePaymentIntentsRetrieve.mockResolvedValue({
        id: 'pi_test_1',
        client_secret: 'pi_test_1_secret',
        ...
        currency: created.currency,
        ...override,
        metadata: { ...created.metadata, ...override.metadata },
      });
  ...
  it('fails closed when retrieving a pending payment fails', async () => {
    expect((await callRoute(makeHeaderlessRequest())).status).toBe(200);
    mocks.stripePaymentIntentsRetrieve.mockRejectedValue(
  ...
  it('supplied keys are isolated across users and resources', () => {
  ...
  it('does not return an unscoped cached payment secret', async () => {
    claimStore.set('shared-client-key', {
      status: 'completed',
      result: {
        clientSecret: 'synthetic_other_payer_secret',
