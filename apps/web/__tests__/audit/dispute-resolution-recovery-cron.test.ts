...
vi.mock('@/lib/services/payment/DisputeResolutionRecoveryService', () => ({
  runDisputeResolutionRecovery: state.run,
}));
...
import { GET } from '@/app/api/cron/dispute-resolution-recovery/route';
const secret = 'synthetic-cron-test-value';
const path = '/api/cron/dispute-resolution-recovery';
const request = (headers: Record<string, string> = {}) =>
  new NextRequest(`http://localhost${path}`, { headers });
describe('dispute resolution recovery cron authentication', () => {
  beforeEach(() => {
    vi.stubEnv('CRON_SECRET', secret);
    vi.stubEnv('CRON_SECRET_DISPUTE_RESOLUTION_RECOVERY', '');
