...
describe('cross-user data isolation (real DB)', () => {
  ...
  beforeAll(async () => {
    ...
    // Audit-only fixture: messages require an assigned recipient, while discovery needs an unassigned job.
    messageJob = await createTestJob({
      homeowner_id: homeownerB.id,
      status: 'draft',
    });
    const assignedMessageJob = await createServiceClient()
      .from('jobs')
      .update({ contractor_id: contractorB.id })
      .eq('id', messageJob.id);
    if (assignedMessageJob.error)
      throw new Error(assignedMessageJob.error.message);
