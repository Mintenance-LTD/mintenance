...
export function YOLOLearningStatusCard() {
  ...
  const { correctionsCount, retrainingStatus, continuousLearningEnabled } =
    status;
  const progress = Math.min(
    100,
    (correctionsCount.approved / retrainingStatus.nextRetrainThreshold) * 100
  );
          ...
          <div className='grid grid-cols-2 gap-2'>
            <div>
              <div className='text-lg font-semibold text-slate-900'>
                {correctionsCount.approved}
              </div>
              <div className='text-xs text-slate-500'>Approved</div>
            </div>
            <div>
              <div className='text-lg font-semibold text-slate-900'>
                {correctionsCount.pending}
              </div>
        ...
        <div>
          <div className='flex items-center justify-between mb-2'>
            ...
            <span className='text-xs text-slate-500'>
              {correctionsCount.approved} /{' '}
              {retrainingStatus.nextRetrainThreshold}
            </span>
          </div>
          ...
          {retrainingStatus.correctionsNeeded > 0 && (
            <p className='text-xs text-slate-500 mt-1'>
              Need {retrainingStatus.correctionsNeeded} more approved
              corrections
            </p>
          )}
        </div>
