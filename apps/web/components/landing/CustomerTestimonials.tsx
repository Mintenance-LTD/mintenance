...
export function CustomerTestimonials() {
  ...
  useEffect(() => {
    async function fetchData() {
      // Fetch testimonials and platform stats in parallel
      const [testimonialsResult, statsResult] = await Promise.allSettled([
        fetch('/api/testimonials').then((r) =>
          r.ok ? r.json() : Promise.reject(r)
        ),
        fetch('/api/stats/platform').then((r) =>
          r.ok ? r.json() : Promise.reject(r)
        ),
      ]);

      if (testimonialsResult.status === 'fulfilled') {
        const data = testimonialsResult.value;
        if (data.testimonials && data.testimonials.length > 0) {
          setTestimonials(data.testimonials);
        }
      } else {
        logger.error('Error fetching testimonials:', testimonialsResult.reason);
      }
      ...
      if (statsResult.status === 'fulfilled') {
        ...
        setPlatformStats({
          avgSavings:
            completedJobs > 0 ? Math.round(totalSaved / completedJobs) : 0,
          avgResponseTimeHours: stats.avgResponseTimeHours ?? 24,
        });
      }
  ...
  return (
    <div className='py-16 bg-gradient-to-b from-white to-slate-50'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        ...
        {/* Stats Bar — real data from /api/stats/platform, falls back to benchmarks */}
        <div className='mt-12 grid grid-cols-3 gap-8 max-w-3xl mx-auto'>
          ...
          <div className='text-center'>
            <div className='text-3xl font-bold text-[#10B981]'>
              {platformStats && platformStats.avgSavings > 0
          ...
          <div className='text-center'>
            <div className='text-3xl font-bold text-[#10B981]'>
              {platformStats
                ? `${Math.round(platformStats.avgResponseTimeHours)}hrs`
                : '24hrs'}
            </div>
