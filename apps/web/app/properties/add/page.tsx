...
import { MintEditorialAddProperty } from './MintEditorialAddProperty';
...
export default function AddPropertyPage2025() {
  ...
  // component above any early returns so the hook order stays stable
  // (same rules-of-hooks pattern we hit earlier on /jobs/create and
  // /messages). The feature-gate and submit handlers below DO have
  // early returns, so this MUST live above them.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.images.length > 0) {
        ...
        if (!uploadRes.ok) {
          const uploadErr = await uploadRes
            .json()
  ...
  if (!propertyAccess.hasAccess && propertyAccess.requiresUpgrade) {
    const limitNum =
      typeof propertyAccess.limit === 'number' ? propertyAccess.limit : 1;
    return (
          ...
          <p className='text-gray-600 mb-8'>
            ...
            {tier === 'free'
              ...
              : tier === 'landlord'
                ? 'Landlord'
                : 'current'}{' '}
            plan allows {limitNum} {limitNum === 1 ? 'property' : 'properties'}.
            {tier === 'free'
