...
export default function PropertyEditClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // updatePropertySchema is `.strict()` — the property id travels in the
      // URL, so it must not leak into the body via the formData spread.
      const { id: _id, photos: _photos, ...updatePayload } = formData;
                ...
                {newFilePreviews.map((preview, index) => (
                  <div key={`new-${index}`} className='relative group'>
                    <div className='aspect-square relative rounded-lg overflow-hidden bg-gray-100 ring-2 ring-teal-400'>
                      ...
                      <span className='absolute bottom-2 left-2 text-xs bg-teal-600 text-white px-2 py-0.5 rounded'>
                        New
                      </span>
                    </div>
                  </div>
