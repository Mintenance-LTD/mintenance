  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(
          err.message ?? 'Team access requires an Agency subscription.'
        );
      } else {
        const err = await res.json();
  ...
  const handleRemove = async (id: string) => {
    try {
      ...
      const res = await fetch(
        `/api/properties/${propertyId}/team?memberId=${id}`,
        {
          method: 'DELETE',
    ...
    <div className='p-4 border border-gray-200 rounded-xl'>
        ...
        <div className='mb-3 p-3 bg-gray-50 rounded-lg space-y-2'>
          ...
          >
            ...
            <option value='manager'>
              Manager — can manage jobs and tenants
            </option>
      ...
      {members.length === 0 ? (
        <p className='text-xs text-gray-500'>
          No team members yet. Invite people to help manage this property.
        </p>
      ) : (
        <div className='space-y-2'>
          {members.map((m) => (
            <div
              key={m.id}
              className='p-2.5 bg-white rounded-lg border border-gray-200 text-xs'
            >
              <div className='flex items-center justify-between'>
                ...
                <button
                  ...
                  className='p-0.5 hover:bg-red-100 rounded ml-2 flex-shrink-0'
                >
              ...
              <div className='flex items-center gap-2 mt-1'>
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] font-medium capitalize ${ROLE_BADGES[m.role] || 'bg-gray-100 text-gray-600'}`}
                >
                  {m.role}
                </span>
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] font-medium capitalize ${STATUS_BADGES[m.status] || 'bg-gray-100 text-gray-600'}`}
                >
                  {m.status}
                </span>
              </div>
