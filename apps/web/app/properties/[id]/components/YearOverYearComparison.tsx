...
function ChangeIndicator({
  ...
  return (
    <span
      className={`flex items-center gap-0.5 text-[10px] ${isUp ? 'text-red-500' : 'text-green-600'}`}
    >
      {isUp ? (
      ...
      <div className='p-4 border border-gray-200 rounded-xl'>
        <div className='flex items-center gap-2 mb-2'>
          ...
          <h4 className='text-sm font-semibold text-gray-900'>
            Job budgets by creation year
          </h4>
        </div>
        <p className='text-xs text-gray-500'>
          Complete some jobs to see year-over-year comparison data.
        </p>
      </div>
    );
  }
    ...
    <div className='p-4 border border-gray-200 rounded-xl'>
      ...
      <div className='grid grid-cols-2 gap-3'>
        ...
        <div className='p-2.5 bg-teal-50 rounded-lg border border-teal-100'>
          <div className='text-[10px] text-teal-600 font-medium mb-1'>
            {currentYear}
          </div>
          <div className='text-lg font-bold text-gray-900'>
            £{currentData.totalSpent.toLocaleString()}
          </div>
          <div className='text-[10px] text-gray-500'>
            {currentData.jobCount} job{currentData.jobCount !== 1 ? 's' : ''}
          </div>
        </div>
        ...
        <div className='p-2.5 bg-gray-50 rounded-lg border border-gray-200'>
          ...
          <div className='text-lg font-bold text-gray-900'>
            £{previousData.totalSpent.toLocaleString()}
          </div>
          <div className='text-[10px] text-gray-500'>
            {previousData.jobCount} job{previousData.jobCount !== 1 ? 's' : ''}
          </div>
        </div>
      </div>
      ...
      <div className='mt-3 space-y-1.5'>
        <div className='flex items-center justify-between text-xs'>
          <span className='text-gray-600'>Budget change</span>
          <ChangeIndicator
            current={currentData.totalSpent}
            previous={previousData.totalSpent}
            format='currency'
          />
        </div>
        <div className='flex items-center justify-between text-xs'>
          ...
          <ChangeIndicator
            current={currentData.jobCount}
            previous={previousData.jobCount}
          />
        </div>
        <div className='flex items-center justify-between text-xs'>
          <span className='text-gray-600'>Categories</span>
          <span className='text-[10px] text-gray-500'>
            {currentData.categories.length} this year
          </span>
        </div>
      </div>
    </div>
