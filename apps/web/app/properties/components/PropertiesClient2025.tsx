...
export function PropertiesClient2025({
  properties,
  propertyLimit,
  tier,
  userInfo,
  ...
  // Track in-flight favorite toggles so a fast double-click can't create a
  // desync between optimistic UI state and the server.
  ...
  const toggleFavorite = async (propertyId: string, e: React.MouseEvent) => {
    ...
    // Ignore clicks while the previous request for this property is still in flight
    if (pendingFavorites.has(propertyId)) return;
    ...
    setFavorites((prev) => {
      ...
      if (wasFavorited) newSet.delete(propertyId);
    ...
    try {
      const response = await fetch(
          ...
          : '/api/properties/favorites',
        {
          ...
          // DELETE uses query string; POST needs the body.
          body: wasFavorited
            ? undefined
      ...
      if (!response.ok) {
        throw new Error(
          `Favorite ${method} failed with status ${response.status}`
        );
      }
      ...
      // Revert to the captured pre-click state — deterministic, ignores any
      // concurrent state changes during the request.
      setFavorites((prev) => {
        const newSet = new Set(prev);
        if (wasFavorited) newSet.add(propertyId);
      ...
      {filteredAndSortedProperties.length === 0 && properties.length === 0 ? (
        <div className='bg-white rounded-xl border border-gray-200 p-12 text-center'>
          ...
          <p className='text-gray-600 mb-6 max-w-md mx-auto'>
            Add your first property to start managing maintenance jobs and
                ...
                <div className='relative aspect-[4/3] bg-gradient-to-br from-slate-100 to-slate-200'>
                  ...
                  {property.activeJobs > 0 && (
                    <div className='absolute top-3 left-3 bg-teal-600 text-white px-3 py-1 rounded-full text-sm font-semibold shadow-md'>
                      ...
                      {property.activeJobs === 1 ? 'Job' : 'Jobs'}
                    </div>
                  )}
                </div>
              ...
              <div className='bg-white rounded-xl overflow-hidden border-2 border-dashed border-gray-300 flex items-center justify-center min-h-[320px]'>
                <div className='text-center p-8'>
                  ...
                  <p className='text-gray-600 text-sm mb-4'>
                    Your {TIER_LABELS[tier] || 'Free'} plan allows{' '}
                    {propertyLimit}{' '}
                    {propertyLimit === 1 ? 'property' : 'properties'}.
                    {tier === 'free'
                      ? ' Upgrade to Landlord for up to 25.'
