...
/** Detect contract-related system messages by their content pattern */
function isContractMessage(content: string): boolean {
  return /contract submitted|contract signed|contract created/i.test(content);
}
          ...
          messages.map((message) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-md ${isCurrentUser ? 'ml-auto' : 'mr-auto'}`}
                >
                  {message.message_type === 'file' && message.attachment_url ? (
                    ...
                    >
                      <div className='flex items-center gap-3'>
                        <div
                          className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isCurrentUser ? 'bg-white/15' : 'bg-teal-50'}`}
                        >
                          <svg
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p
                            className={`text-xs mt-0.5 ${isCurrentUser ? 'text-white/60' : 'text-gray-500'}`}
                          >
                            Shared document
                          </p>
                        </div>
                    ...
                    (() => {
                      const isContract = isContractMessage(message.content);
                      const bubbleClasses = `px-4 py-2.5 rounded-2xl ${
                        isCurrentUser
                          ? 'bg-teal-700 text-white rounded-br-sm'
                          : 'bg-teal-50 text-teal-900 rounded-bl-sm border border-teal-200'
                      } ${isContract ? 'cursor-pointer hover:ring-2 hover:ring-teal-400 transition-shadow' : ''}`;
                      ...
                      const bubbleContent = (
                        <>
                          ...
                          {isContract && (
                            <p
                              className={`text-xs mt-1.5 font-medium ${isCurrentUser ? 'text-teal-200' : 'text-teal-600'}`}
                            >
