...
export function JobSignOffClient({
  jobId,
  jobTitle,
  contractorName,
  currentUserRole,
}: JobSignOffClientProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!uploadError && uploadData) {
        // Bucket is private — use a long-lived signed URL for stored signature reference.
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to submit sign-off.'
      );
    } finally {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Please review the work for <strong>{jobTitle}</strong> by{' '}
          <strong>{contractorName}</strong> and sign below to complete the job.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {error && (
