...
export default async function JobTrackingPage() {
  ...
  // 2026-05-09: the embedded-resource name was `users!` from the
  // pre-2025 schema; the FK now targets `profiles` (the `users` table
  // was dropped). PostgREST embeds the foreign table by name, so this
  // query 400'd in production and the contractor avatar/name never
  // rendered. The constraint name is unchanged.
  const { data: jobsData } = await serverSupabase
    .from('jobs')
    .select(
      `
      id, title, status, budget, category, created_at, updated_at, contractor_id,
      contractor:profiles!jobs_contractor_id_fkey ( id, first_name, last_name, profile_image_url )
    `
    )
  ...
  const selectedJob = selectedJobRaw
    ? {
        ...selectedJobRaw,
        contractor: Array.isArray(selectedJobRaw.contractor)
          ? selectedJobRaw.contractor[0]
          : selectedJobRaw.contractor,
      }
    : undefined;
  ...
  const progressPercent = selectedJob
    ? getProgressFromStatus(selectedJob.status)
    : 0;
  ...
  if (selectedJob) {
    const { data: messagesData } = await serverSupabase
      .from('messages')
      .select(
        `id, content, created_at, sender_id, sender:profiles!messages_sender_id_fkey ( id, first_name, last_name, profile_image_url )`
      )
  ...
  return (
    ...
    >
      <TrackingHeader
        userProfileImageUrl={
          (user as unknown as { profile_image_url?: string }).profile_image_url
        }
      />
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Monitor the real-time status and progress of your active home
              maintenance jobs.
            </p>
          </div>
        </div>
                      ...
                      {selectedJob.contractor?.profile_image_url ? (
                        ...
                        >
                          {selectedJob.contractor
                            ? selectedJob.contractor.first_name?.[0] || 'C'
                            : '?'}
                        </div>
                      )}
                      <div>
                        ...
                        >
                          by{' '}
                          {selectedJob.contractor
                            ? `${selectedJob.contractor.first_name} ${selectedJob.contractor.last_name}`
                            : 'Unassigned'}
                        </p>
                      </div>
                    </div>
