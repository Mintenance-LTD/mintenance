...
export const JobCard2025 = React.memo(
  function JobCard2025({
    job,
    viewMode = 'grid',
    ...
    return (
      <Link
        href={selectionMode ? '#' : `/jobs/${job.id}`}
      ...
      >
        <MotionArticle
          className={`bg-white rounded-2xl border shadow-sm overflow-hidden cursor-pointer relative ${
            ...
            isSelected
              ? 'border-teal-500 border-2 ring-2 ring-teal-200'
              : 'border-gray-200'
        ...
        >
          ...
          {!selectionMode && (
            <JobCardQuickActions
              jobId={job.id}
              status={job.status}
              previousContractorId={job.contractor_id ?? null}
            />
          )}
          ...
          {job.photos && job.photos.length > 0 ? (
            <div
              className={`relative bg-gray-100 ${viewMode === 'list' ? 'w-48 h-full' : 'h-48'}`}
            >
              <Image
                src={job.photos[0]}
                alt={job.title}
              ...
              {job.ai_assessment && (
                <div className='absolute top-2 left-2'>
                  ...
                  {job.ai_assessment.severity && (
                    <div
                      className={`mt-1.5 inline-flex items-center gap-1 px-2 py-1 rounded-md ${
                    ...
                    >
                      <span
                        className={`w-1.5 h-1.5 rounded-full ${
                          getSeverityConfig(job.ai_assessment.severity)
                            ?.dotBg || ''
                        }`}
                      />
          ...
          <div className={`p-6 ${viewMode === 'list' ? 'flex-1' : ''}`}>
            ...
            {(!job.photos || job.photos.length === 0) && job.description && (
            ...
            <div className='flex items-center gap-2 flex-wrap mb-4'>
              ...
              {job.priority && (
                <span
                  className={`px-3 py-1 bg-gray-100 rounded-full text-xs font-semibold ${getPriorityColor(job.priority)}`}
                >
            ...
            {job.ai_assessment && (
              <div className='flex items-center gap-3 mb-4 p-3 bg-gradient-to-r from-indigo-50/50 to-purple-50/50 rounded-lg border border-indigo-100'>
                {/* Cost Estimate */}
                {job.ai_assessment.assessment_data?.contractorAdvice
                  ?.estimatedCost && (
                      ...
                      <div
                        ...
                        className={`w-1.5 h-3 rounded-sm ${
                          ...
                          ['low', 'medium', 'high'].indexOf(
                            ...
                            ? job.ai_assessment?.assessment_data
                                ?.contractorAdvice?.complexity === 'high'
                              ? 'bg-red-400'
                              : job.ai_assessment?.assessment_data
                                    ?.contractorAdvice?.complexity === 'medium'
                                ? 'bg-amber-400'
                                : 'bg-green-400'
                            : 'bg-gray-200'
                        }`}
                ...
                {job.ai_assessment.assessment_data?.safetyHazards
                  ?.hasCriticalHazards && (
