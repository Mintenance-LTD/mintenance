...
function SmartJobFilters2025({
  onFilterChange,
  initialFilters,
}: SmartJobFilters2025Props) {
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            {/* Tabs */}
            <div
              className='flex items-center gap-2 p-4 bg-gray-50 border-b border-gray-200 overflow-x-auto'
              role='tablist'
                  ...
                  {activeTab === 'category' && (
                    <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
                      {categoryOptions.map((option) => {
                        const isSelected =
                          filters.category?.includes(option.value) || false;
                        return (
                          <MotionButton
                            key={option.value}
                            onClick={() =>
                              toggleArrayFilter('category', option.value)
                            }
                  ...
                  {activeTab === 'urgency' && (
                    <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
                      {urgencyOptions.map((option) => {
                        const isSelected =
                          filters.urgency?.includes(option.value) || false;
                        const urgencyColors = {
                        ...
                        return (
                          <MotionButton
                            key={option.value}
                            onClick={() =>
                              toggleArrayFilter('urgency', option.value)
                            }
                            className={`p-4 rounded-xl border-2 text-center transition-all ${
                              isSelected
                                ? urgencyColors[
                                    option.value as keyof typeof urgencyColors
                                  ]
        ...
        <div className='p-4 bg-gray-50 border-t border-gray-200'>
          <div className='flex items-center gap-2 flex-wrap'>
            ...
            {filters.status?.map((status) => (
              <span
                key={status}
                className='px-2 py-1 bg-teal-100 text-teal-700 rounded-lg text-xs font-medium'
              >
                {status.replace('_', ' ')}
              </span>
            ))}
