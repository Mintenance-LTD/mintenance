...
// Server-side fetch against `profiles` runs at render time. Next 16 +
// Turbopack prerenders this page at build time without the runtime
// Supabase env vars available, which throws:
//   "[Supabase] Missing NEXT_PUBLIC_SUPABASE_URL environment variable"
// Marking the page dynamic defers the fetch until a real request,
// which is semantically correct — the contractor list changes per
// user / over time and doesn't benefit from static generation.
export const dynamic = 'force-dynamic';

export const metadata: Metadata = {
  title: 'Find Trusted Contractors | Mintenance',
  description:
    'Connect with verified professionals for your home improvement projects. Browse contractors by specialty, location, and ratings.',
  openGraph: {
    title: 'Find Trusted Contractors | Mintenance',
    description:
...
export default async function FindContractorsPage() {
  ...
  reviewsData.data?.forEach((review) => {
    if (!contractorStats.has(review.contractor_id)) {
      contractorStats.set(review.contractor_id, {
        ratings: [],
        completedJobs: 0,
      });
    }
