...
export function ContactsClient({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // /api/landlord/contacts uses the default `csrf: true` on
      // withApiHandler — without an X-CSRF-Token the POST 403s in
      // production. Audit 2026-05-12 caught this. The Mint Editorial
      // port already sent CSRF; aligning the legacy path now.
      ...
      <div className='flex items-center justify-between mb-8'>
        <div>
          ...
          <p className='mt-1 text-gray-500'>
            Manage tenant and keyholder records.
          </p>
        </div>
        <button
          type='button'
      ...
      {filtered.length === 0 ? (
        <div className='text-center py-16 bg-white rounded-xl border border-gray-200'>
          ...
          <p className='text-gray-500'>
            Add tenants, keyholders and emergency contacts.
          </p>
        </div>
      ) : (
              ...
              <div className='flex items-center justify-between'>
                <div className='flex items-center gap-3'>
                  <div className='w-10 h-10 bg-blue-50 rounded-full flex items-center justify-center text-blue-600'>
                    {ROLE_ICONS[contact.contact_role] || (
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-3 text-xs text-gray-500'>
                      <span className='capitalize'>
                        {ROLE_LABELS[contact.contact_role] ||
                          contact.contact_role}
                      </span>
                      {contact.unit_label && (
                        <span>Unit {contact.unit_label}</span>
                      )}
