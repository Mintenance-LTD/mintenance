    ...
    const fetchActiveBids = async () => {
      ...
      try {
        ...
        const stats = {
          ...
          pendingBids: bidsWithJobs.filter(
            (b: ActiveBid) => b.status === 'pending'
          ).length,
          acceptedBids: bidsWithJobs.filter(
            (b: ActiveBid) => b.status === 'accepted'
          ).length,
          totalValue: bidsWithJobs.reduce(
            (sum: number, b: ActiveBid) => sum + (b.amount || 0),
            0
          ),
        };
        setStats(stats);
                ...
                >
                  <div className='flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4'>
                        ...
                        {bid.jobs?.homeowner?.profile_image_url ? (
                          ...
                          <div className='w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center flex-shrink-0'>
                            <span className='text-lg font-medium text-gray-600'>
                              {bid.jobs?.homeowner?.first_name?.charAt(0) ||
                                'H'}
                            </span>
                          </div>
                        )}
                        <div className='flex-1 min-w-0'>
                          ...
                          <div className='flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500'>
                            <span className='flex items-center gap-1'>
                              ...
                              {bid.jobs?.location || 'Unknown'}
                            </span>
                            <span className='flex items-center gap-1'>
                              <Briefcase className='w-4 h-4' />{' '}
                              {bid.jobs?.category || 'General'}
                            </span>
                            <span className='hidden sm:flex items-center gap-1'>
                              <User className='w-4 h-4' />{' '}
                              {bid.jobs?.homeowner?.first_name}{' '}
                              {bid.jobs?.homeowner?.last_name}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className='flex items-center justify-between sm:block sm:text-right sm:ml-4 border-t sm:border-t-0 pt-3 sm:pt-0 sm:flex-shrink-0'>
                      <div
                        className={`inline-block px-3 py-1 rounded-full text-xs font-medium sm:mb-2 ${getStatusColor(bid.status)}`}
                      >
                        {bid.status.charAt(0).toUpperCase() +
                          bid.status.slice(1)}
                      </div>
                      <div className='text-right'>
                        <div className='text-xs text-gray-600 sm:mb-1'>
                          Your Bid
                        </div>
