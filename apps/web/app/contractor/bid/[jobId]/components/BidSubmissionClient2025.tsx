...
export function BidSubmissionClient2025(props: BidSubmissionClient2025Props) {
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>(
    existingBid?.lineItems?.map((item) => ({
      ...item,
  ...
  const [estimatedDuration, setEstimatedDuration] = useState<number | ''>(
    existingBid?.estimatedDuration || ''
  );
  const [proposedStartDate, setProposedStartDate] = useState(
    existingBid?.proposedStartDate || ''
  );
  ...
  const handleGetPricingSuggestion = async () => {
    try {
      ...
      if (!response.ok) {
        toast.error(
          data.message || data.error || 'Failed to get pricing suggestion'
        );
        return;
      }
      ...
      logger.error('Error fetching pricing suggestion:', error, {
        service: 'ui',
      });
  ...
  const handleSubmit = async () => {
    ...
    if (description.trim().length < 50) {
      toast.error(
        'Proposal must be at least 50 characters (currently ' +
    ...
    if (job.budget) {
      ...
      if (!isNaN(jobBudget) && totalAmount > jobBudget) {
        ...
        const maxBase =
          taxRate > 0
            ? (jobBudget / (1 + taxRate / 100)).toFixed(2)
            : jobBudget.toFixed(2);
        let errorMsg =
        ...
        if (taxAmount > 0)
          errorMsg +=
            ...
            '% tax (£' +
            taxAmount.toFixed(2) +
            ').\nMaximum base amount (before tax): £' +
            maxBase;
    ...
    try {
      ...
      const startDateValue =
        proposedStartDate && proposedStartDate.trim() !== ''
      ...
      const requestPayload = {
        ...
        ...(startDateValue !== undefined && {
          proposedStartDate: startDateValue,
        }),
      };
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(
            'Failed to submit bid: ' + (response.statusText || 'Unknown error')
          );
        }
        let errorMessage =
          errorData.error ||
        ...
        if (
          errorMessage.includes('cannot exceed job budget') ||
          errorMessage.includes('exceeds job budget')
        ) {
          const excessMatch = errorMessage.match(/£(\d+\.?\d*).*£(\d+\.?\d*)/);
          if (excessMatch) {
            const b = parseFloat(excessMatch[1]);
            const bg = parseFloat(excessMatch[2]);
            const ex = b - bg;
            errorMessage =
              'Your total bid amount (£' +
              b.toFixed(2) +
              ...
              '.';
            errorMessage +=
              lineItems.length > 0 || taxAmount > 0
                ? ' This includes your line items and ' +
                  taxRate +
                  '% tax. Please adjust your pricing to stay within the budget.'
                : ' Please reduce your bid amount to stay within the budget.';
          }
        }
      ...
      toast.error(errorMessage, {
        duration:
          errorMessage.includes('exceeds') || errorMessage.includes('budget')
            ? 8000
            : 5000,
            ...
            >
                  ...
                  >
                    <div>
                      ...
                      <div className='relative'>
                        <span className='pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 text-lg leading-none'>
                          £
                        </span>
                        <input
                          type='number'
                          ...
                          step='0.01'
                          min='0'
                          className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-lg ${!amount ? 'border-gray-300' : 'border-teal-300'}`}
                          required
                        />
                      </div>
                      {!amount && (
                    ...
                    {amount && parseFloat(amount) > 0 && (
                      <div className='bg-gray-50 border border-gray-200 rounded-xl p-6'>
                        ...
                        <p className='text-xs text-gray-500 mt-4'>
                          The platform fee covers payment processing, dispute
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 ${!description ? 'border-gray-300' : description.trim().length < 50 ? 'border-rose-300' : 'border-teal-300'}`}
                  maxLength={5000}
                  required
                />
                <div
                  className={`text-sm mt-1 ${!description ? 'text-rose-600' : description.trim().length < 50 ? 'text-rose-600' : 'text-teal-600'}`}
                >
                  {!description ? (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (v === '') setEstimatedDuration('');
                      else {
                        const n = parseInt(v, 10);
                        setEstimatedDuration(isNaN(n) ? '' : n);
                      }
                    }}
                    placeholder='e.g., 2'
                    min='1'
                ...
                <button
                  onClick={(e) => {
                    ...
                    if (!submitting) handleSubmit();
                      ...
                      toast.error(
                        'Please wait, your bid is already being submitted...'
                      );
                  }}
                  ...
                  title={
                    !amount || !description
                      ? 'Please fill in bid amount and description to submit'
                      : ''
                  }
                >
                  {submitting ? (
