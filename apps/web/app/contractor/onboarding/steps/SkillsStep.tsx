...
export function SkillsStep({ data, onNext, onBack, saving }: Props) {
  const [form, setForm] = useState<SkillsData>(data);
  const [errors, setErrors] = useState<Partial<Record<keyof SkillsData, string>>>({});
  ...
  function validate(): boolean {
    ...
    if (form.selectedSkills.length === 0) next.selectedSkills = 'Select at least one skill.';
    setErrors(next);
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      <h2 className="text-xl font-semibold text-gray-900 mb-1">Skills &amp; Qualifications</h2>
      <p className="text-sm text-gray-500 mb-6">Select the services you offer. This helps match you with the right jobs.</p>
      ...
      <div className="mb-5">
        ...
        {errors.selectedSkills && <p className="text-xs text-red-500 mt-2">{errors.selectedSkills}</p>}
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          />
          <p className="text-xs text-gray-400 mt-1">Optional — increases trust with homeowners</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
          <p className="text-xs text-gray-400 mt-1">Optional — shows homeowners you&apos;re covered</p>
        </div>
      </div>
