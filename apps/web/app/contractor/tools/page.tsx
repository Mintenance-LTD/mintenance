...
export default function ToolsEquipmentPage() {
      ...
      const res = await fetch('/api/contractor/tools', {
        ...
        body: JSON.stringify({
          ...
          currentValue: formCurrentValue
            ? parseFloat(formCurrentValue)
            : formPurchasePrice
              ? parseFloat(formPurchasePrice)
              : 0,
  ...
  const valueByCategory = useMemo(() => {
    const grouped = tools.reduce(
      (acc, tool) => {
        const cat =
          CATEGORIES.find((c) => c.value === tool.category)?.label || 'Other';
      ...
      {isMintEditorial ? (
        ...
        >
          <div className='col' style={{ gap: 4 }}>
            <h1 className='t-h1'>Tools & equipment</h1>
            <p className='t-body'>
              Track your tool inventory, current value, maintenance windows, and
              assignments. Add new items as you grow your kit.
            </p>
          </div>
          <button
            type='button'
            ...
            <div className='bg-white rounded-xl border border-gray-200 p-6'>
              ...
              <BarChart
                data={[
                  ...
                  {
                    status: 'In Use',
                    count: tools.filter((t) => t.status === 'in_use').length,
                  },
                  { status: 'Maintenance', count: stats.maintenance },
                  {
                    status: 'Retired',
                    count: tools.filter((t) => t.status === 'retired').length,
                  },
                ]}
                index='status'
                categories={['count']}
                  ...
                  <div className='flex items-start gap-4 flex-1'>
                    ...
                    <div className='flex-1'>
                      <div className='flex items-center gap-3 mb-2'>
                        ...
                        <span
                          className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(tool.status)}`}
                        >
                      ...
                      <div className='flex flex-wrap gap-x-6 gap-y-1 text-sm'>
                        <span className='text-gray-500'>
                          Condition:{' '}
                          <span
                            className={`font-medium ${getConditionColor(tool.condition)}`}
                          >
                            {tool.condition.charAt(0).toUpperCase() +
                              tool.condition.slice(1)}
                          </span>
                        </span>
                        {tool.location && (
                          <span className='text-gray-500'>
