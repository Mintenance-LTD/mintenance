...
export function DiscoverClient({
  ...
  const progressPercentage =
    items.length > 0 ? Math.round((currentIndex / items.length) * 100) : 0;
  ...
  const handleSwipe = useCallback(
    async (action: 'like' | 'pass' | 'super_like' | 'maybe') => {
  ...
  return (
    ...
    >
      ...
      {error && (
        ...
        >
          <Icon name='xCircle' size={20} color='var(--me-on-brand)' />
          <span>{error}</span>
        </div>
      )}
        ...
        {!hasMoreItems ? (
          ...
          <>
            ...
            {showUndo && swipeHistory.length > 0 && (
              ...
              >
                <Icon name='arrowLeft' size={16} color='var(--me-ink)' />
                Undo
              </button>
            )}
