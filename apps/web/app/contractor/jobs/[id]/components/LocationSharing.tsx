...
export function LocationSharing({ jobId, contractorId }: LocationSharingProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Using a ref avoids the stale closure issue with useState
  ...
  const startLocationSharing = () => {
      ...
      async (position) => {
        try {
          const response = await fetchWithCsrf(
            `/api/contractors/${contractorId}/location`,
            {
              method: 'POST',
              headers: {
    ...
    watchIdRef.current = watchIdValue;
  };
  ...
  const stopLocationSharing = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to stop location sharing'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : 'Failed to enable location sharing'
        );
        setIsLoading(false);
      }
    }
