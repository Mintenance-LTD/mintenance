...
export default function ContractorJobsPage2025() {
  ...
  const [filter, setFilter] = useState<
    'active' | 'viewed' | 'saved' | 'bid' | 'completed'
  >('active');
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const pending = allJobs.filter(
          (j: JobApiResponse) => j.status === 'pending' || j.status === 'posted'
        ).length;
        const completed = allJobs.filter(
          (j: JobApiResponse) => j.status === 'completed'
        ).length;
        const totalValue = allJobs.reduce(
          (sum: number, j: JobApiResponse) => sum + (j.budget || 0),
          0
        );
      ...
      >
        ...
        <div className='grid grid-cols-4 gap-4'>
          {[
            {
              label: 'Active Jobs',
              value: loadingStats ? '...' : allJobsStats.active,
            },
            {
              label: 'Pending Bids',
              value: loadingStats ? '...' : allJobsStats.pending,
            },
            {
              label: 'Completed',
              value: loadingStats ? '...' : allJobsStats.completed,
            },
            {
              label: 'Total Value',
              value: loadingStats
                ? '...'
                : `£${allJobsStats.totalValue.toFixed(0)}`,
            },
          ].map((stat) => (
            <MotionDiv
              key={stat.label}
