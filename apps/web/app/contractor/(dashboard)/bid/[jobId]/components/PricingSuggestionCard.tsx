    ...
    >
        ...
        <div className='grid grid-cols-3 gap-4'>
          ...
          <div className='bg-gradient-to-br from-teal-500 to-emerald-600 rounded-xl p-4 text-center shadow-md transform scale-105'>
            ...
            <p className='text-3xl font-bold text-white'>
              £{priceRange.recommended.toFixed(2)}
            </p>
            <div className='mt-2'>
        ...
        <div className='grid grid-cols-2 gap-4'>
          <div>
            ...
            <p className='text-lg font-semibold text-gray-900'>
              £{marketData.averageBid.toFixed(2)}
            </p>
          </div>
          <div>
            <p className='text-xs text-gray-500'>Median Bid</p>
            <p className='text-lg font-semibold text-gray-900'>
              £{marketData.medianBid.toFixed(2)}
            </p>
          </div>
          <div>
            <p className='text-xs text-gray-500'>Market Range</p>
            <p className='text-sm font-medium text-gray-900'>
              £{marketData.rangeMin.toFixed(2)} - £
              {marketData.rangeMax.toFixed(2)}
            </p>
          </div>
          <div>
            <p className='text-xs text-gray-500'>Competitiveness</p>
            <span
              className={`inline-block px-2 py-1 rounded-md text-xs font-semibold ${colors.badge}`}
            >
      ...
      {suggestion.costBreakdown && (
        <div className='mb-6 bg-white rounded-xl p-4 border border-gray-200'>
          ...
          <div
            className='mb-4 h-8 flex rounded-lg overflow-hidden'
            role='img'
            aria-label='Cost breakdown visualization'
          >
            {suggestion.costBreakdown.materials > 0 && (
              <div
                ...
                style={{
                  width: `${(suggestion.costBreakdown.materials / suggestion.costBreakdown.total) * 100}%`,
                }}
                title={`Materials: £${suggestion.costBreakdown.materials.toFixed(2)}`}
              >
                {(suggestion.costBreakdown.materials /
            ...
            {suggestion.costBreakdown.labor > 0 && (
              <div
                ...
                style={{
                  width: `${(suggestion.costBreakdown.labor / suggestion.costBreakdown.total) * 100}%`,
                }}
                title={`Labor: £${suggestion.costBreakdown.labor.toFixed(2)}`}
              >
                {(suggestion.costBreakdown.labor /
                  suggestion.costBreakdown.total) *
                  100 >=
                  10 &&
            ...
            {suggestion.costBreakdown.overhead > 0 && (
              <div
                ...
                style={{
                  width: `${(suggestion.costBreakdown.overhead / suggestion.costBreakdown.total) * 100}%`,
                }}
                title={`Overhead: £${suggestion.costBreakdown.overhead.toFixed(2)}`}
              >
                {(suggestion.costBreakdown.overhead /
                  suggestion.costBreakdown.total) *
                  100 >=
                  10 &&
            ...
            {suggestion.costBreakdown.profit > 0 && (
              <div
                ...
                style={{
                  width: `${(suggestion.costBreakdown.profit / suggestion.costBreakdown.total) * 100}%`,
                }}
                title={`Profit: £${suggestion.costBreakdown.profit.toFixed(2)}`}
              >
                {(suggestion.costBreakdown.profit /
                  suggestion.costBreakdown.total) *
                  100 >=
                  10 &&
            ...
            <div className='flex items-center gap-2'>
              ...
              <div className='flex-1 min-w-0'>
                ...
                <p className='text-sm font-semibold text-gray-900'>
                  £{suggestion.costBreakdown.materials.toFixed(2)}
                </p>
              </div>
            </div>
            <div className='flex items-center gap-2'>
              ...
              <div className='flex-1 min-w-0'>
                ...
                <p className='text-sm font-semibold text-gray-900'>
                  £{suggestion.costBreakdown.labor.toFixed(2)}
                </p>
              </div>
            </div>
            <div className='flex items-center gap-2'>
              ...
              <div className='flex-1 min-w-0'>
                ...
                <p className='text-sm font-semibold text-gray-900'>
                  £{suggestion.costBreakdown.overhead.toFixed(2)}
                </p>
              </div>
            </div>
            <div className='flex items-center gap-2'>
              ...
              <div className='flex-1 min-w-0'>
                ...
                <p className='text-sm font-semibold text-gray-900'>
                  £{suggestion.costBreakdown.profit.toFixed(2)}
                </p>
              </div>
            </div>
          </div>
      ...
      {factors &&
        (factors.complexityFactor ||
          ...
          factors.contractorTierFactor) && (
          <details className='mt-4 text-xs text-gray-600'>
            <summary className='cursor-pointer hover:text-gray-900 font-medium'>
              View adjustment factors
            </summary>
            <div className='mt-2 space-y-1 pl-4'>
              {factors.complexityFactor !== undefined && (
                <p>
                  • Complexity factor:{' '}
                  {(factors.complexityFactor * 100).toFixed(0)}%
                </p>
              )}
              {factors.locationFactor !== undefined && (
                <p>
                  • Location factor: {(factors.locationFactor * 100).toFixed(0)}
                  %
                </p>
              )}
              {factors.contractorTierFactor !== undefined && (
                <p>
                  • Contractor tier factor:{' '}
                  {(factors.contractorTierFactor * 100).toFixed(0)}%
                </p>
              )}
              {factors.marketDemandFactor !== undefined && (
                <p>
                  • Market demand factor:{' '}
                  {(factors.marketDemandFactor * 100).toFixed(0)}%
                </p>
              )}
            </div>
          </details>
