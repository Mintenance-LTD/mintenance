...
export function BidSubmissionClient({ job }: BidSubmissionClientProps) {
  ...
  const handleSubmit = async () => {
    if (!amount || !description) {
      setFeedback({
        type: 'error',
        message: 'Please add a bid amount and short proposal.',
      });
      return;
    }
    ...
    if (!job.id) {
      setFeedback({
        type: 'error',
        message: 'Invalid job. Please try again from the jobs page.',
      });
      return;
    }
    ...
    if (description.trim().length > 5000) {
      setFeedback({
        type: 'error',
        message: 'Proposal description cannot exceed 5000 characters.',
      });
      return;
    }
    ...
    try {
      ...
      logger.info('Submitting bid', {
        ...
        proposalLength: description.trim().length,
      });
      ...
      try {
        ...
        if (responseText && responseText.trim()) {
          try {
            ...
            if (!response.ok) {
              throw new Error(
                `Server error (${response.status}): ${response.statusText || 'Unknown error'}. ${responseText.substring(0, 200)}`
              );
            }
            throw new Error(
        ...
        if (!response.ok) {
          ...
          const errorMsg =
            statusMessages[response.status] ||
            `Server error (${response.status}): ${response.statusText || 'Failed to read response'}`;
          throw new Error(errorMsg);
        }
        throw readError;
      }
      ...
      if (!response.ok) {
        ...
        const getStatusMessage = (status: number): string => {
          ...
          return (
            ...
            `Request failed with status ${status}: ${response.statusText || 'Unknown error'}`
          );
        };
        ...
        if (isEmptyResponse) {
          ...
          logger.error('EMPTY RESPONSE - Throwing status-based error', {
            defaultMessage,
          });
        ...
        if (responseData.details && Array.isArray(responseData.details)) {
          const errorMessages = (responseData.details as ValidationError[]).map(
            (err: ValidationError) => {
        ...
        if (!hasValidErrorMessage) {
          const defaultMessage = getStatusMessage(response.status);
          logger.error('NO VALID ERROR - Throwing status-based error', {
            defaultMessage,
          });
          throw new Error(defaultMessage);
        }
        ...
        if (
          ...
          const defaultMessage = getStatusMessage(response.status);
          logger.error(
            'DETECTED GENERIC ERROR MESSAGE - Replacing with status-based error',
            { defaultMessage }
          );
          throw new Error(defaultMessage);
        }
      ...
      if (
        ...
        errorMessage =
          'An error occurred while submitting your bid. Please try again or contact support if the issue persists.';
        logger.error(
          'Generic error message detected in catch block. This indicates an unexpected error flow.'
        );
      }
    ...
    >
      ...
      >
        <div>
          ...
          >
            Review the project details and include your timeline, pricing, and
      ...
      {feedback && (
        ...
        >
          ...
          <AlertDescription
            className={feedback.type === 'success' ? 'text-green-700' : ''}
          >
            {feedback.message}
          </AlertDescription>
          <button
        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: showAdvancedQuote
                  ? theme.colors.primary
                  : 'transparent',
                color: showAdvancedQuote
          ...
          {!showAdvancedQuote ? (
            <>
              ...
              >
                <Lightbulb size={16} style={{ flexShrink: 0 }} /> Tip: Use
                "Advanced Quote Details" to break down your pricing with line
                items, taxes, and terms.
              </div>
            </>
          ) : (
            <QuoteLineItems
          ...
          {description && (
            ...
            >
              ...
              {description.trim().length >= 50 && <CheckCircle2 size={14} />}
            </div>
          )}
