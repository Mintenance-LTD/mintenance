      ...
      {jobContext && (
        ...
        >
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <div className='flex items-center gap-4 text-xs text-slate-600'>
                  <span
                    className={`px-3 py-1 rounded-xl font-semibold ${getStatusColor(jobContext.status)}`}
                  >
                    {jobContext.status.replace('_', ' ')}
                  </span>
                  {/* 2026-05-22: budget chip removed — contractors price each bid themselves. */}
                  {jobContext.deadline && (
                    <div className='flex items-center gap-1.5'>
                      <Calendar className='w-4 h-4' />
                      <span>
                        Due{' '}
                        {new Date(jobContext.deadline).toLocaleDateString(
                          'en-GB'
                        )}
                      </span>
                    </div>
            ...
            <button
              onClick={() => onViewJob(jobContext.id)}
              ...
              {messages.map((message, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`flex flex-col max-w-md ${isCurrentUser ? 'items-end' : 'items-start'}`}
                    >
                      {message.message_type === 'file' &&
                      message.attachment_url ? (
                        <div
                          className={`px-5 py-3.5 rounded-2xl shadow-sm ${isCurrentUser ? 'bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-br-md' : 'bg-white text-slate-900 rounded-bl-md border border-slate-200'}`}
                        >
                          <div className='flex items-center gap-3'>
                            <div
                              className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isCurrentUser ? 'bg-white/10' : 'bg-teal-50'}`}
                            >
                              <FileText
                                className={`w-5 h-5 ${isCurrentUser ? 'text-white/80' : 'text-teal-600'}`}
                              />
                            </div>
                            <div className='flex-1 min-w-0'>
                              ...
                              <p
                                className={`text-xs mt-0.5 ${isCurrentUser ? 'text-white/60' : 'text-slate-500'}`}
                              >
                                Shared document
                              </p>
                            </div>
                        ...
                        <div
                          className={`px-5 py-3.5 rounded-2xl shadow-sm ${isCurrentUser ? 'bg-gradient-to-r from-teal-700 to-teal-800 text-white rounded-br-md' : 'bg-teal-50 text-teal-900 rounded-bl-md border border-teal-200'}`}
                        >
                          <div className='flex items-start gap-2'>
                            <ClipboardList
                              className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isCurrentUser ? 'text-teal-200' : 'text-teal-600'}`}
                            />
                      ...
                      <div
                        className={`flex items-center gap-2 mt-1.5 text-xs ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'}`}
                      >
                        ...
                        {isCurrentUser && (
                          <CheckCheck
                            className={`w-4 h-4 ${message.read ? 'text-teal-500' : 'text-slate-400'}`}
                          />
                        )}
                      </div>
                    </div>
