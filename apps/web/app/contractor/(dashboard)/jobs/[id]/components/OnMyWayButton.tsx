...
export function OnMyWayButton({
    ...
    try {
      ...
      if (res.ok) {
        ...
        const activeTrip = data.trips?.find(
          (t: Trip & { job_id?: string }) =>
            t.job_id === jobId && t.status === 'en_route'
        );
        if (activeTrip) {
          setTrip(activeTrip);
          return;
      ...
      if (arrivedRes.ok) {
        ...
        const arrivedTrip = arrivedData.trips?.find(
          (t: Trip & { job_id?: string }) =>
            t.job_id === jobId && t.status === 'arrived'
        );
  ...
  const startTrip = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errData.error?.message || errData.error || 'Failed to start trip'
        );
      }
  ...
  const updateTrip = async (status: 'arrived' | 'cancelled') => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(
          errData.error?.message || errData.error || 'Failed to update trip'
        );
      }
        ...
        <div className='flex items-center gap-3'>
          ...
          <div>
            ...
            <p className='text-sm text-emerald-600'>
              Arrived at{' '}
              {trip.arrived_at
                ? new Date(trip.arrived_at).toLocaleTimeString('en-GB', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })
                : 'location'}
  ...
  if (trip?.status === 'en_route') {
    const elapsed = Math.round(
      (Date.now() - new Date(trip.started_at).getTime()) / 60000
    );
    return (
    ...
    <div className='bg-white border-2 border-slate-200 rounded-2xl p-5'>
      <div className='mb-3'>
        ...
        <p className='text-sm text-slate-500'>
          Let the homeowner and admin know you&apos;re on your way
        </p>
      </div>
      <Button
        onClick={startTrip}
