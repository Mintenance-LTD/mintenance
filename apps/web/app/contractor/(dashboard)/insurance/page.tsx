...
export default function InsuranceLicensingPage() {
  ...
  const handleAddLicense = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Failed to add licence'
      );
    } finally {
      setSubmitting(false);
    }
    ...
    if (type === 'insurance') {
      ...
      try {
        ...
        const res = await fetch(
          `/api/contractor/insurance?id=${id}&type=insurance`,
      ...
      try {
        const csrfHeaders = await getCsrfHeaders();
        const res = await fetch(
          `/api/contractor/insurance?id=${id}&type=license`,
          { method: 'DELETE', headers: csrfHeaders, credentials: 'include' }
        );
        if (!res.ok) throw new Error('Failed');
  ...
  return (
    <div className='min-h-0 bg-gray-50'>
      {/* Header — canonical .t-h1 + .btn-primary when Mint Editorial,
          legacy Tailwind banner otherwise. */}
      {isMintEditorial ? (
        <div
          className='between'
        ...
        >
          <div className='col' style={{ gap: 4 }}>
            <h1 className='t-h1'>Insurance & licensing</h1>
            <p className='t-body'>
              Keep your public liability, professional indemnity, and trade
              licences up to date so homeowners trust you with bigger jobs.
            </p>
          </div>
          <button
            type='button'
            ...
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
              <div>
                ...
                <p className='text-gray-600 mt-1'>
                  Manage your business insurance and professional licences
                </p>
              </div>
              <button
                onClick={() => setShowAddModal(true)}
      ...
      <div className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
        ...
        >
          ...
          >
            ...
            <p className='text-2xl font-bold text-gray-900'>
              {insurances.filter((i) => i.status === 'expiring_soon').length}
            </p>
          </MotionDiv>
          <MotionDiv
            variants={staggerItem}
            ...
            <nav className='flex gap-8 px-6'>
              {(['insurance', 'licenses'] as const).map((tab) => (
                <button
                  ...
                  className={`py-4 border-b-2 font-medium transition-colors ${activeTab === tab ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-600 hover:text-gray-900'}`}
                >
                ...
                {insurances.length === 0 && (
                  <div className='text-center py-12'>
                    ...
                    <p className='text-gray-600'>
                      Add your first insurance policy to track coverage
                    </p>
                  </div>
                )}
                {insurances.map((ins) => {
                  ...
                  return (
                    ...
                    >
                      <div className='flex items-start justify-between gap-4'>
                          ...
                          <div className='flex items-center gap-3 mb-3'>
                            ...
                            <div>
                              <div className='flex items-center gap-3 mb-1'>
                                ...
                                <span
                                  className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(ins.status)}`}
                                >
                        ...
                        <button
                          onClick={() =>
                            handleDelete(ins.id, 'insurance', ins.type)
                          }
              ...
              <div className='space-y-6'>
                {licenses.length === 0 && (
                  <div className='text-center py-12'>
                    ...
                    <p className='text-gray-600'>
                      Add your professional licences and certifications
                    </p>
                  </div>
                )}
                {licenses.map((lic) => {
                  const daysUntilExpiry = lic.expiryDate
                    ? calculateDaysUntilExpiry(lic.expiryDate)
                    : null;
                  return (
                    <MotionDiv
                      key={lic.id}
                    ...
                    >
                      <div className='flex items-start justify-between gap-4'>
                          ...
                          <div className='flex items-center gap-3 mb-3'>
                            ...
                            <div>
                              <div className='flex items-center gap-3 mb-1'>
                                ...
                                <span
                                  className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(lic.status)}`}
                                >
                        ...
                        <button
                          onClick={() =>
                            handleDelete(lic.id, 'license', lic.name)
                          }
                          className='p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors'
                        >
                          <Trash2 className='w-5 h-5' />
            ...
            {activeTab === 'insurance' ? (
              <div className='space-y-4'>
                ...
                <div>
                  ...
                  <input
                    type='text'
                    value={insContactName}
                    onChange={(e) => setInsContactName(e.target.value)}
                    className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent'
                  />
                </div>
                <div className='grid grid-cols-2 gap-4'>
                  <div>
                    ...
                    <input
                      type='tel'
                      value={insContactPhone}
                      onChange={(e) => setInsContactPhone(e.target.value)}
                      className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent'
                    />
                  </div>
                  <div>
              ...
              <div className='space-y-4'>
                ...
                <div>
                  ...
                  <input
                    type='text'
                    value={licIssuer}
                    onChange={(e) => setLicIssuer(e.target.value)}
                    className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent'
                    placeholder='e.g. Gas Safe Register'
                  />
                </div>
