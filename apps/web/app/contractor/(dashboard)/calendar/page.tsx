...
export default function ContractorCalendarPage() {
  ...
  const loadAvailability = useCallback(async () => {
    const map = await fetchSavedAvailability();
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      await Promise.all([loadEvents(), loadAvailability()]);
      setLoading(false);
    };
    load();
  }, [loadEvents, loadAvailability]);
  ...
  const availableSlots = Object.values(availability).filter(
    (s) => s.enabled
  ).length;
              ...
              <div className='grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden'>
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(
                  (day) => (
                    <div
                      key={day}
                      className='bg-gray-50 text-center font-medium text-gray-700 py-3 text-sm'
                    >
                      {day}
                    </div>
                  )
                  ...
                  return (
                    ...
                    >
                      {day && (
                        <div>
                          <div
                            className={`text-sm mb-2 flex items-center justify-center w-7 h-7 rounded-full mx-auto ${
                              isCurrentDay
                          ...
                          <div className='space-y-1'>
                            {dayEvents.slice(0, 2).map((event) => (
                              <div
                                key={event.id}
                                className={`px-2 py-1 rounded text-xs ${getEventColor(event.type)}`}
                              >
