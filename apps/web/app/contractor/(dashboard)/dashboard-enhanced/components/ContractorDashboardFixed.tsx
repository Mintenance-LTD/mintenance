...
export function ContractorDashboardFixed({
  data,
}: ContractorDashboardFixedProps) {
  const { contractor, metrics, recentJobs, notifications } = data;

  const calculateResponseRate = () => {
    const totalBids =
      metrics.pendingBids + metrics.activeJobs + metrics.completedJobs;
    if (totalBids === 0) return 0;
    return Math.round(
      ((metrics.activeJobs + metrics.completedJobs) / totalBids) * 100
    );
  };
