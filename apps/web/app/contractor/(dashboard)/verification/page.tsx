...
export default function ContractorVerificationPage() {
  ...
  return (
    <ContractorPageWrapper>
      {/* Hero Header — canonical .t-h1 + brand-soft icon tile + mint
          progress bar when Mint Editorial is on. */}
      {isMintEditorial ? (
        <div className='card card-pad' style={{ marginBottom: 24 }}>
          <div
            className='row'
            style={{ gap: 14, alignItems: 'center', marginBottom: 20 }}
          >
            <span
              style={{
                width: 56,
            ...
            <div className='col' style={{ gap: 4 }}>
              <h1 className='t-h1'>Verification centre</h1>
              <p className='t-body'>
                Complete identity, insurance, and trade qualifications
                verification to unlock priority placement and higher-value jobs.
              </p>
            </div>
          </div>
          <div
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; label: string }> = {
    approved: {
      bg: 'bg-emerald-100 text-emerald-700 border-emerald-600',
      label: 'Approved',
    },
    pending: {
      bg: 'bg-amber-100 text-amber-700 border-amber-600',
      label: 'Under Review',
    },
    rejected: {
      bg: 'bg-rose-100 text-rose-700 border-rose-600',
      label: 'Rejected',
    },
    not_started: {
      bg: 'bg-gray-100 text-gray-700 border-gray-600',
      label: 'Not Started',
    },
  };
  ...
  return (
    <span
      className={`px-3 py-1 rounded-lg text-sm font-semibold border ${c.bg}`}
    >
      {c.label}
    </span>
  );
