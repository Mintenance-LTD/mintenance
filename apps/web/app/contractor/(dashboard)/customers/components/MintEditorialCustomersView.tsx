...
function StarRow({ value }: { value: number | null }) {
  if (value === null) {
    return <span style={{ color: 'var(--me-ink-4)', fontSize: 13 }}>—</span>;
  }
...
export function MintEditorialCustomersView({
  ...
  const filtered = useMemo(
    () =>
      filter === 'all'
        ? customers
        : customers.filter((c) => c.stage === filter),
    [customers, filter]
  );
            ...
            <tbody>
              {filtered.map((c) => (
                ...
                >
                  ...
                  <td
                    style={{ padding: '14px 16px', color: 'var(--me-ink-2)' }}
                  >
                    {c.property ?? (
                  ...
                  >
                    {c.lastJobTitle ? (
                      <span>
                        ...
                        {c.lastJobDate ? (
                          <span
                            style={{ color: 'var(--me-ink-3)', marginLeft: 6 }}
                          >
