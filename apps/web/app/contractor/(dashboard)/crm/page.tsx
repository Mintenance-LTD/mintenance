...
export const metadata: Metadata = {
  ...
  description:
    'Manage your client relationships, track jobs, and view analytics for your contracting business.',
};
...
export default async function CRMDashboardPage() {
  ...
  payments?.forEach((payment) => {
    if (payment.status === 'completed') {
      ...
      const matchingClient = Array.from(clientsMap.values()).find(
        (client) =>
          client.id === payment.payer_id || client.email === payment.payer_id
      );
  ...
  const newClientsThisMonth = clients.filter(
    (client) => new Date(client.first_job_date) >= currentMonth
  ).length;
