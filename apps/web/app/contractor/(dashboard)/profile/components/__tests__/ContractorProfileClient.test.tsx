...
vi.mock('../ProfileStats', () => ({
  ...
  PerformanceSnapshot: () => (
    <div data-testid='performance-snapshot'>Performance Snapshot</div>
  ),
}));

vi.mock('../ProfileGallery', () => ({
  ProfileGallery: () => (
    <div data-testid='profile-gallery'>Profile Gallery</div>
  ),
}));

vi.mock('../ProfileReviews', () => ({
  ProfileReviews: () => (
    <div data-testid='profile-reviews'>Profile Reviews</div>
  ),
}));

vi.mock('../ProfileQuickActions', () => ({
  ProfileQuickActions: () => (
    <div data-testid='profile-quick-actions'>Profile Quick Actions</div>
  ),
}));

vi.mock('../EditProfileDialog', () => ({
  EditProfileDialog: () => (
    <div data-testid='edit-profile-dialog'>Edit Profile Dialog</div>
  ),
}));

vi.mock('../SkillsManagementDialog', () => ({
  SkillsManagementDialog: () => (
    <div data-testid='skills-management-dialog'>Skills Management Dialog</div>
  ),
}));

vi.mock('../PhotoUploadDialog', () => ({
  PhotoUploadDialog: () => (
    <div data-testid='photo-upload-dialog'>Photo Upload Dialog</div>
  ),
}));

vi.mock('../ContractorDataPrivacy', () => ({
  ContractorDataPrivacy: () => (
    <div data-testid='contractor-data-privacy'>Contractor Data Privacy</div>
  ),
}));
