...
export const GET = withApiHandler(
  ...
  async (_req, { user }) => {
    ...
    const contractDocs = (contracts || []).map((c: Record<string, unknown>) => {
      const contractor = c.contractor as { first_name?: string; last_name?: string; company_name?: string } | null;
      ...
      return {
        ...
        name: (c.title as string) || 'Contract Agreement',
    ...
    const jobTitleMap = new Map((jobs || []).map((j: { id: string; title: string }) => [j.id, j.title]));
    ...
    if (jobIds.length > 0) {
      ...
      bidDocs = (bids || []).map((b: Record<string, unknown>) => {
        const contractor = b.contractor as { first_name?: string; last_name?: string; company_name?: string } | null;
        const contractorName = contractor?.company_name
          || (contractor?.first_name && contractor?.last_name
            ? `${contractor.first_name} ${contractor.last_name}`
            : 'Contractor');
