 ...
 * Get current feature flag status and metrics (admin only)
 */
export const GET = withApiHandler(
  ...
  async (request) => {
    ...
    // Get all flags
    const allFlags = getAllFlagsConfig();
 ...
 * Record metrics for feature flag performance or trigger rollback (admin only)
 */
export const POST = withApiHandler(
  ...
  async (request) => {
    ...
    if (flag && reason) {
      return postRollbackResponse(validation.data as Record<string, unknown>);
    }
...
/** GET /api/feature-flags?status=true */
...
/** POST rollback handler */
async function postRollbackResponse(body: Record<string, unknown>): Promise<NextResponse> {
