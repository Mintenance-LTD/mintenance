...
export const GET = withApiHandler(
  ...
  async (request) => {
    ...
    const { count: predictionCount } = await serverSupabase
    ...
    const { data: performanceSnapshots } = await serverSupabase
      .from('model_performance_snapshots')
      .select('timestamp, metrics, performance_score')
      .gte('timestamp', startDate.toISOString())
      .order('timestamp', { ascending: true });

    // 3. Routing Statistics (limited sample)
    ...
    // 4. Cost Savings
    const internalModelUsage =
      routingDecisions?.filter((d) => d.route_selected === 'internal').length ||
      0;

    const gpt4Usage =
      routingDecisions?.filter((d) => d.route_selected === 'gpt4_vision')
        .length || 0;
    ...
    const correctedCount =
      corrections?.filter((c) => !c.was_correct).length || 0;
    const totalFeedback = accurateCount + correctedCount;

    // 7. Learning Velocity (predictions per day) — use limited sample to estimate
    const { data: dailyPredictionsSample } = await serverSupabase
      .from('model_predictions_log')
      .select('created_at')
      .gte('created_at', startDate.toISOString())
    ...
    // 9. Class Distribution (limited sample)
    const { data: classDistribution } = await serverSupabase
      .from('model_predictions_log')
      .select('damage_type')
      .gte('created_at', startDate.toISOString())
      .limit(5000);
    ...
    const improvementRate =
      firstSnapshot && lastSnapshot
        ? ((lastSnapshot.performance_score - firstSnapshot.performance_score) /
            firstSnapshot.performance_score) *
          100
        : 0;
    ...
    return NextResponse.json({
      ...
      costSavings: {
        ...
        projectedMonthlySavings:
          Math.round((savedCost / days) * 30 * 100) / 100,
      },
      userFeedback: {
        totalFeedback,
        accurateCount,
      ...
      learningVelocity: {
        predictionsPerDay:
          Object.values(predictionsByDay).reduce((a, b) => a + b, 0) /
      ...
      recommendations: generateRecommendations({
        ...
        improvementRate,
        accuracyRate:
          totalFeedback > 0 ? (accurateCount / totalFeedback) * 100 : 100,
        internalUsage:
          totalRequests > 0 ? (internalModelUsage / totalRequests) * 100 : 0,
      }),
    });
  }
);
...
function generateRecommendations(metrics: {
  ...
  if (metrics.highQualityCount < 1000) {
    ...
    recommendations.push('Ready for model retraining with accumulated data');
  }

  if (metrics.improvementRate < 0) {
    recommendations.push(
      'Model performance declining - urgent retraining recommended'
    );
  } else if (metrics.improvementRate > 5) {
    recommendations.push('Model improving well - continue current strategy');
  }

  if (metrics.accuracyRate < 80) {
    recommendations.push(
      'High correction rate - review model predictions and adjust thresholds'
    );
  }
  ...
  if (metrics.internalUsage < 50) {
    ...
    recommendations.push('Good balance - 70%+ using internal model');
  }
