  ...
  async () => {
    ...
    const mrr =
      mrrData && mrrData.length > 0
        ? parseFloat(mrrData[0].total_mrr || '0')
        : 0;

    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    const [{ data: userGrowthData }, { data: jobGrowthData }] =
      await Promise.all([
        serverSupabase
          .from('profiles')
          .select('created_at')
    ...
    const userGrowthCumulative = Object.entries(userGrowthByDay).map(
      ([date, count]) => {
    ...
    const jobGrowthCumulative = Object.entries(jobGrowthByDay).map(
      ([date, count]) => {
