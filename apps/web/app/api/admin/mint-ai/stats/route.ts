...
export const GET = withApiHandler({ roles: ['admin'] }, async () => {
  ...
  const shadowStats = {
    ...
    parseSuccessRate:
      shadows.length > 0
        ? shadows.filter((r) => r.student_parse_success).length / shadows.length
        : 0,
    avgLatencyMs:
      shadows.length > 0
        ? shadows.reduce((s, r) => s + (r.latency_ms ?? 0), 0) / shadows.length
        : 0,
  ...
  const bufferStats = {
    ...
    byQuality: {
      ...
      uncertain: buffer.filter((b) => b.teacher_quality === 'uncertain').length,
    },
    byCategory: Object.entries(
