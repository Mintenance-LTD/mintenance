...
import {
  ...
  SettingCategory,
} from '@/lib/services/admin/PlatformSettingsService';
...
export const GET = withApiHandler(
  ...
  async (request) => {
    ...
    if (category) {
      const settings = await PlatformSettingsService.getSettingsByCategory(
...
export const PUT = withApiHandler(
  {
    roles: ['admin'],
    rateLimit: false,
    // Platform-wide settings — toggling these (e.g. fee rates,
    // feature flags) on a stolen session would have system-wide
    // blast radius. Demand fresh MFA proof.
  ...
  async (request, { user }) => {
    ...
    if (expectedType === 'number' && valueType !== 'number') {
      throw new BadRequestError(
        `Invalid value type. Expected ${expectedType}, got ${valueType}`
      );
    }
    ...
    if (
      (expectedType === 'json' || expectedType === 'array') &&
      valueType !== 'object'
    ) {
      throw new BadRequestError(
        `Invalid value type. Expected object/array, got ${valueType}`
      );
    }
...
export const POST = withApiHandler(
  {
    ...
    // Same blast radius as PUT — creating new platform settings is
    // equivalent to enabling new feature flags / fee schedules.
    requireMfaVerifiedWithinMinutes: 15,
  },
  async (request, { user }) => {
    // SECURITY: Verify admin role from database for sensitive operations
    ...
    const { key, value, type, category, description, isPublic } =
      validation.data;

    const valueType = typeof value;
    if (type === 'number' && valueType !== 'number') {
      throw new BadRequestError(
        `Invalid value type. Expected number, got ${valueType}`
      );
    }
    if (type === 'boolean' && valueType !== 'boolean') {
      throw new BadRequestError(
        `Invalid value type. Expected boolean, got ${valueType}`
      );
    }
    if (type === 'string' && valueType !== 'string') {
      throw new BadRequestError(
        `Invalid value type. Expected string, got ${valueType}`
      );
    }
