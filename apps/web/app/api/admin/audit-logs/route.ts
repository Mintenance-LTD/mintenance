 ...
 *
 * Actual audit_logs table columns: id, user_id, action, ip_address, created_at
 * We normalise the response to match the client-side AuditLog interface by
 * deriving missing fields from the `action` column.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Build query using only columns that exist in the table
    ...
    // Map to the shape the client expects, deriving fields from `action`
    ...
    logger.error('Unexpected error in audit logs API', {
      service: 'admin-audit-logs',
...
/** Derive a resource_type from the action string */
...
/** Derive a status from the action string */
