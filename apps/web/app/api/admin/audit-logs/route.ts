...
export const GET = withApiHandler(
  ...
  async (request) => {
    ...
    // admin_activity_log is the table AdminActivityLogger writes to
    let query = serverSupabase
      .from('admin_activity_log')
      .select(
        'id, admin_id, action_type, action_category, target_type, target_id, description, metadata, ip_address, user_agent, created_at',
        { count: 'exact' }
      )
    ...
    const logs = rawLogs.map((row) => ({
      ...
      resource_type: row.target_type || deriveResourceType(row.action_type),
      resource_id: row.target_id || null,
      details: row.description,
      ip_address: row.ip_address,
      user_agent: row.user_agent || null,
      created_at: row.created_at,
      status: deriveStatus(row.action_type),
