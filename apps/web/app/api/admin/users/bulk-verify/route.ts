...
export const POST = withApiHandler(
  {
    ...
    // Bulk-verifying users is a trust-grant primitive — a stolen
    // session must not flip 100 accounts at once. Demand fresh MFA.
  ...
  async (request, { user: admin }) => {
    ...
    if (action === 'reject' && !reason) {
      throw new BadRequestError(
        'Reason is required when rejecting verifications'
      );
    }
    ...
    for (const userId of userIds) {
      try {
        ...
        const automatedCheck =
          await VerificationService.automatedVerificationCheck(userId);
