...
} from '@/lib/services/cache/AIResponseCache';
...
export const POST = withApiHandler(
  {
    ...
    // 2026-05-01 audit follow-up: drops cached AI responses, forcing the
  ...
  async (request, { user }) => {
    ...
    if (!confirm) {
      throw new BadRequestError(
        'Confirmation required. Set "confirm": true in request body to clear cache'
      );
    }
...
export const GET = withApiHandler(
  ...
  async () => {
    const metrics = AIResponseCache.exportMetrics() as {
      aggregated: { totalCacheSize: number; totalSavedCost: number };
      perService: unknown;
    };
    ...
    return NextResponse.json({
      ...
      warning:
        'Clearing cache will temporarily increase API costs until cache is rebuilt',
    });
  }
);
