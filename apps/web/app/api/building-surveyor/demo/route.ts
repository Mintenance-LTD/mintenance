    ...
    .array(
      z
        ...
        .refine(
          ...
          { message: 'Must be an HTTP/HTTPS URL or base64 image data URI' }
        )
    )
    .min(1)
    .max(3),
...
export const POST = withApiHandler(
  ...
  async (request) => {
    ...
    let rateLimiter: Awaited<
      typeof import('@/lib/rate-limiter')
    >['rateLimiter'];
    let serverSupabase: Awaited<
      typeof import('@/lib/api/supabaseServer')
    >['serverSupabase'];
    let BuildingSurveyorService: Awaited<
      typeof import('@/lib/services/building-surveyor')
    >['BuildingSurveyorService'];

    try {
      logger = (await import('@mintenance/shared')).logger;
      rateLimiter = (await import('@/lib/rate-limiter')).rateLimiter;
      serverSupabase = (await import('@/lib/api/supabaseServer'))
        .serverSupabase;
      BuildingSurveyorService = (
        await import('@/lib/services/building-surveyor')
      ).BuildingSurveyorService;
    } catch (importError) {
      fallbackLogger.error(
        'Demo route: failed to load dependencies',
