...
export const POST = withApiHandler(
  ...
  async (request, { user }) => {
    ...
    const response: {
      ...
      assessment: unknown;
      route: unknown;
      agreementScore: unknown;
      fallbacksUsed: unknown;
      fusionDetails?: { [key: string]: unknown };
      modelOutputs?: { [key: string]: unknown };
    ...
    if (options?.includeFusionDetails) {
      response.fusionDetails = {
        ...
        refinedBoxesCount:
          result.fusionOutput.refinedBoundingBoxes?.length || 0,
        metrics: {
          ...
          effectiveSampleSize:
            result.fusionOutput.fusionMetrics.effectiveSampleSize,
        },
      };
    ...
    if (options?.includePerformanceMetrics) {
      const availability =
        EnhancedHybridInferenceService.getServiceAvailability();
...
export const GET = withApiHandler({}, async (request, { user }) => {
  ...
  logger.info('Fusion stats retrieved', {
    service: 'building-surveyor-fusion',
    userId: user.id,
    type,
  });
