...
export async function handlePut(
  ...
  const { data: existingJob, error: fetchError } = await userDb
    .from('jobs')
    .select(
      'homeowner_id, status, location, title, description, city, postcode'
    )
    .eq('id', jobId)
    .single();
  ...
  if (existingJob.status !== 'posted') {
    throw new BadRequestError(
      'Cannot edit job that is already in progress or completed'
    );
  }
  ...
  // Audit step 4 finish (2026-04-29): coalesce the deprecated alias
  // fields (`priority` → `urgency`, `images` → `photoUrls`) into
  // their canonical names up front so the rest of the handler
  // doesn't have to keep checking both.
  const photoUrls: string[] | undefined = payload.photoUrls ?? payload.images;
  ...
  if (photoUrls && photoUrls.length > 0) {
    ...
    // Mirror the validated list back onto both alias slots so the
    // downstream blocks reading `payload.images` see the cleaned
    // URLs too.
  ...
  if (payload.analyzeWithAI) {
    try {
      const cacheInput = {
        jobId,
        images: photoUrls || [],
        runBuildingSurvey: payload.runBuildingSurvey ?? false,
      };
      aiAnalysisResult = await AIResponseCache.get(
        ...
        async () => {
          ...
          if (payload.runBuildingSurvey && photoUrls && photoUrls.length > 0) {
            ...
            const { BuildingSurveyorService } =
              await import('@/lib/services/building-surveyor/BuildingSurveyorService');
      ...
      buildingSurveyResult =
        (aiAnalysisResult as { buildingSurvey: unknown } | null)
          ?.buildingSurvey ?? null;
    } catch (aiError) {
  ...
  if (payload.location || payload.city || payload.postcode) {
    const fullAddress =
      `${payload.location || ''}, ${payload.city || ''}, ${payload.postcode || ''}`.trim();
  ...
  // Audit step 4 finish (2026-04-29): write to `urgency` not
  // `priority`. The previous code wrote to a non-existent column —
  // every "edit job urgency" save was silently a no-op. The
  // coalesce above handles both alias names.
  ...
  if (payload.budgetMax !== undefined)
    updatePayload.budget_max = payload.budgetMax || payload.budget;
  ...
  if (payload.status && updatedJob.status !== oldStatus && currentJob) {
    try {
      ...
      await notifyJobStatusChange({
        ...
        contractorId:
          currentJob.contractor_id || updatedJob.contractor_id || null,
      });
  ...
  // Update job attachments if images changed. `photoUrls` was
  // resolved up top from `payload.photoUrls ?? payload.images`, so
  // both alias names trigger the rebuild.
  if (photoUrls !== undefined) {
    ...
    if (photoUrls.length > 0) {
      ...
      await userDb.from('job_attachments').insert(attachments);
    }
  }
