...
export const GET = withApiHandler(
  ...
  async (request) => {
    ...
    if (filters.category) {
      const result = await materialsService.getMaterialsByCategory(
        filters.category,
        filters
      );
    ...
    if (filters.search) {
      // SECURITY: Sanitize before .ilike() — this route is unauthenticated,
      // so raw interpolation allowed PostgREST filter injection. See
      // lib/utils/sanitize-postgrest.ts.
      const safeSearch = sanitizeIlikePattern(filters.search);
      if (safeSearch) {
