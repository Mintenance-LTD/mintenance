...
// service accounts that should never be invoking user-scoped
// automation) could enter the handler. Admins triage agents via
// /admin/* surfaces; this endpoint is for end-user automation.
export const POST = withApiHandler(
  { roles: ['homeowner', 'contractor'], rateLimit: { maxRequests: 30 } },
  async (req, { user }) => {
    let validatedData;
    try {
    ...
    switch (agentName) {
      ...
      case 'NotificationAgent':
        decision = await (
          NotificationAgent as unknown as {
      ...
      case 'DisputeResolutionAgent':
        decision = await (
          DisputeResolutionAgent as unknown as {
            analyzeDispute(
              j: string,
              d: unknown
            ): Promise<Record<string, unknown>>;
    ...
    await AgentLogger.logDecision({
      ...
      actionTaken: ((decisionRecord?.action as string) ||
        'evaluated') as ActionTaken,
      confidence: (decisionRecord?.confidence as number) || 0.75,
      reasoning:
        (decisionRecord?.reasoning as string) || 'API request processed',
      metadata: {
        ...(decisionRecord ?? {}),
        userId: user.id,
    ...
    return NextResponse.json(
      ...
      },
      {
        headers: {
          ...
          'X-Agent': agentName,
          'X-Confidence': (
            (decisionRecord?.confidence as number) || 0
          ).toString(),
          'X-Automation-Level': userPrefs?.automation_level || 'minimal',
        },
      }
