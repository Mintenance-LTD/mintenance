...
export const POST = withApiHandler(
  ...
  async (req, { user }) => {
    ...
    switch (action) {
      case 'evaluate': {
        const { data: bids } = await serverSupabase
          .from('bids')
          .select(
            `*, contractor:contractor_id (id, first_name, last_name, rating, total_jobs, certifications)`
          )
        ...
        const evaluations = await Promise.all(
          bids.map(async (bid: Record<string, unknown>) => {
            ...
            const evaluation = await (
              BidAcceptanceAgent as unknown as {
                evaluateBid(
                  jobId: string,
                  contractorId: string,
                  data: unknown
            ...
            ).evaluateBid(context.jobId, bid.contractor_id as string, {
      ...
      case 'auto-accept': {
        if (!context.bidId)
          throw new BadRequestError('bidId required for auto-accept');
        ...
        const evaluation = await (
          BidAcceptanceAgent as unknown as {
            evaluateBid(
              ...
              contractorId: string,
              data: unknown
            ): Promise<{ confidence: number; recommend: boolean }>;
          }
        ).evaluateBid(context.jobId, bid.contractor_id, {
          bidAmount: bid.amount,
      ...
      case 'recommend': {
        const { data: allBids } = await serverSupabase
          .from('bids')
          .select(
            `*, contractor:contractor_id (id, first_name, last_name, rating, total_jobs, certifications, profile_image_url)`
          )
          .eq('job_id', context.jobId)
          .eq('status', 'pending');
        ...
        const recommendations = await Promise.all(
          allBids.map(async (bid: Record<string, unknown>) => {
            const bidContractorRec = bid.contractor as Record<
              string,
              unknown
            > | null;
            const eval_ = await (
        ...
        const bestBid = recommendations.reduce(
          (best: Record<string, unknown>, current: Record<string, unknown>) =>
            ((current.score as number) || 0) > ((best.score as number) || 0)
              ? current
              : best
        );
    ...
    await AgentLogger.logDecision({
      ...
      actionTaken: (action === 'auto-accept' &&
      (result as Record<string, unknown>).accepted
        ? 'accepted'
        : 'evaluated') as ActionTaken,
      confidence:
        ((result as Record<string, unknown>).confidence as number) || 0.75,
      reasoning:
    ...
    logger.info('Bid acceptance agent processed', {
      service: 'api',
      action,
      userId: user.id,
    });
