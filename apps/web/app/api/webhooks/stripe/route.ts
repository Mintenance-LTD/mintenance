...
async function handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent) {
  ...
  try {
    ...
    // Try both payment_intent_id and stripe_payment_intent_id for compatibility
    ...
    // If escrow transaction found but missing payer_id/payee_id, backfill from payment intent metadata
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  ...
  try {
    ...
    // If escrow transaction found but missing payer_id/payee_id, backfill from job
    if (escrowTransaction && (!escrowTransaction.payer_id || !escrowTransaction.payee_id)) {
