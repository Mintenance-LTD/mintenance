...
// auth-check: ok — auth: false because the route refreshes a soon-to-
// expire session; the refresh-cookie is the auth.
      ...
      return NextResponse.json(
        {
          ...
          message:
            'You have extended your session too many times. Please wait 30 minutes or login again.',
        },
        {
          status: 429,
          headers: {
        ...
        return NextResponse.json(
          {
            ...
            message:
              'Your session has reached the maximum duration (12 hours). Please login again.',
    ...
    await securityMonitor
      .logSuspiciousActivity(
        ...
        user.id,
        {
          ...
          sessionAgeHours: sessionStart
            ? Math.floor((Date.now() - sessionStart) / (60 * 60 * 1000))
            : null,
          idleMinutesBeforeExtension: lastActivity
            ? Math.floor((Date.now() - lastActivity) / (60 * 1000))
            : null,
          newExpiresAt,
        }
      )
