...
// auth-check: ok — same chicken-and-egg pattern as /api/auth/refresh:
// the client uses this to ask "am I logged in?" before deciding what
// to render. Returns 401 when no session cookie is present.
export const GET = withApiHandler(
  { auth: false, rateLimit: false },
  async (request) => {
    const isDev = process.env.NODE_ENV === 'development';

    logger.debug('[session-status] Rate limit check', {
      isDev,
    ...
    const sessionStart = (user as unknown as { sessionStart?: number })
      .sessionStart;
    ...
    if (!sessionStart || !lastActivity) {
      return NextResponse.json({
        ...
        expiresAt: null,
        timeoutType: null,
        warnings: { shouldWarnSoon: false, shouldWarnCritical: false },
    ...
    const warningThresholdMs =
      parseInt(process.env.SESSION_WARNING_THRESHOLD_MINUTES || '5') *
      60 *
      1000;
    const criticalThresholdMs =
      parseInt(process.env.SESSION_CRITICAL_THRESHOLD_MINUTES || '1') *
      60 *
      1000;

    const warnings = {
      shouldWarnSoon:
        timeRemainingMs <= warningThresholdMs &&
        timeRemainingMs > criticalThresholdMs,
    ...
    logger.debug('Session status checked', {
      ...
      timeoutType,
      warningLevel: warnings.shouldWarnCritical
        ? 'critical'
        : warnings.shouldWarnSoon
          ? 'warning'
          : 'normal',
    });
