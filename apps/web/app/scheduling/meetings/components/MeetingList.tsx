...
export function MeetingList(props: MeetingListProps) {
  ...
  useEffect(() => {
    ...
    fetchMeetings();
    const cleanup = subscribeToMeetings();
    return cleanup;
  }, [userId]);
  ...
  const fetchMeetings = async () => {
    try {
      const roleColumn =
        userRole === 'homeowner' ? 'homeowner_id' : 'contractor_id';
      ...
      interface MeetingDbResponse {
        ...
        job?:
          ...
          | Array<{ id: string; title: string; location: string }>;
      }

      if (data) {
        // Map DB response to frontend type
        const mappedMeetings: ContractorMeeting[] = data.map(
          (m: MeetingDbResponse) => ({
            id: m.id,
            jobId: m.job_id,
            ...
            homeowner: (Array.isArray(m.homeowner)
              ? m.homeowner[0]
              : m.homeowner) as User | undefined,
            contractor: (Array.isArray(m.contractor)
              ? m.contractor[0]
              : m.contractor) as User | undefined,
  ...
  const upcomingMeetings = meetings.filter(
    (m) =>
      new Date(m.scheduledDateTime) >= new Date() && m.status !== 'cancelled'
  );
...
function MeetingCard({
  ...
  const otherParty =
    userRole === 'contractor' ? meeting.homeowner : meeting.contractor;
  ...
  return (
    <div
      className={`bg-white rounded-xl border-l-4 ${isPast ? 'border-gray-300' : 'border-teal-600'} border-t border-r border-b border-gray-200 p-6 hover:shadow-md transition-shadow`}
    >
      <div className='flex justify-between items-start flex-wrap gap-6'>
        <div className='flex-1 min-w-[250px]'>
          <div className='flex items-center gap-2 mb-3'>
            <span
              className={`px-3 py-1 rounded-lg text-xs font-medium ${isPast ? 'bg-gray-100 text-gray-700' : 'bg-teal-100 text-teal-700'}`}
            >
        ...
        <div className='flex flex-col gap-3 min-w-[200px]'>
          <div className='flex items-center gap-3'>
            ...
            <span className='font-medium text-gray-900'>
              {new Date(meeting.scheduledDateTime).toLocaleDateString(
                undefined,
          ...
          <div className='flex items-center gap-3'>
            ...
            <span className='text-gray-700'>
              {new Date(meeting.scheduledDateTime).toLocaleTimeString(
                undefined,
