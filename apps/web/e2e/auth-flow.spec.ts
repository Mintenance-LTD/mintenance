...
test.describe('Authentication Flow', () => {
  test.describe('Sign Up Flow', () => {
    test('user can create new account with valid details', async ({ page }) => {
      ...
      await page
        .getByLabel(/email/i)
        .fill(`test${timestamp}${randomSuffix}@example.com`);
      ...
      const hasAccountCreated = await page
        .getByText('Account created successfully!')
        .isVisible()
        .catch(() => false);
      const hasCheckEmail = await page
        .getByText(/Please check your email/i)
        .isVisible()
        .catch(() => false);
      const hasRateLimitError = await page
        .getByText(/rate limit|too many/i)
        .isVisible()
        .catch(() => false);
    ...
    test('validates passwords match', async ({ page }) => {
      ...
      await expect(
        page.getByText(/passwords.*not match|passwords must match/i)
      ).toBeVisible();
    });
  ...
  test.describe('Login Flow', () => {
    ...
    test('provides "Sign Up" link for new users', async ({ page }) => {
      ...
      const signUpLink = page
        .getByRole('link', {
          name: /sign up|create account|create a new account/i,
        })
        .first();
      ...
      if (!isVisible) {
        const hasSignupText = await page
          .getByText(/sign up|create account|create a new account/i)
          .isVisible()
          .catch(() => false);
