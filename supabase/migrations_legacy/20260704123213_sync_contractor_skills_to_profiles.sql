...
-- the denormalized profiles.skills array in step. SECURITY DEFINER so
-- mobile's direct client-side inserts sync despite the profiles
-- column-grant lock. COALESCE(NEW, OLD) matters: the onboarding save
-- route deletes all rows before reinserting, and NEW is NULL in DELETE
-- triggers.
