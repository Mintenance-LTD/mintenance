...
BEGIN
  ...
  IF v_role = 'contractor' THEN
    ...
    -- Drop contractor-side child rows that FK directly to profiles.
  ...
  ELSE
    -- Homeowner: hard-delete owned jobs and their RESTRICT/NO ACTION
    -- dependants before deleting the jobs themselves.
    ...
    -- Homeowner-side child rows that FK to profiles (not via jobs).
  ...
  -- Now NULL out every remaining RESTRICT / NO ACTION FK reference to
  -- profiles that isn't in the owner-tables allow-list above. This
  -- covers all the "actor" columns on audit/log/admin tables — we
  -- keep the historical row, just lose the actor reference. Without
  -- this loop a single audit_log.user_id row would block deletion.
  FOR rec IN
    SELECT
