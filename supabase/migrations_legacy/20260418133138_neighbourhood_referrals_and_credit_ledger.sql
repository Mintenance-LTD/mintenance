...
CREATE POLICY neighbourhood_referrals_referrer_manage
  ON public.neighbourhood_referrals FOR ALL TO authenticated
...
CREATE POLICY neighbourhood_referrals_referred_read
  ON public.neighbourhood_referrals FOR SELECT TO authenticated
...
CREATE POLICY neighbourhood_referrals_service_role
  ON public.neighbourhood_referrals FOR ALL TO service_role
...
CREATE POLICY user_credits_own_read ON public.user_credits FOR SELECT TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS user_credits_service_role ON public.user_credits;
CREATE POLICY user_credits_service_role ON public.user_credits FOR ALL TO service_role
  USING (TRUE) WITH CHECK (TRUE);
...
DROP POLICY IF EXISTS user_credit_ledger_own_read ON public.user_credit_ledger;
CREATE POLICY user_credit_ledger_own_read ON public.user_credit_ledger FOR SELECT TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS user_credit_ledger_service_role ON public.user_credit_ledger;
CREATE POLICY user_credit_ledger_service_role ON public.user_credit_ledger FOR ALL TO service_role
  USING (TRUE) WITH CHECK (TRUE);
