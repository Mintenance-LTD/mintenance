...
DROP POLICY IF EXISTS "Anyone can view contractor skills" ON public.contractor_skills;
CREATE POLICY "Anyone can view contractor skills" ON public.contractor_skills
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Contractors can manage their own skills" ON public.contractor_skills;
CREATE POLICY "Contractors can manage their own skills" ON public.contractor_skills
  FOR ALL USING (contractor_id = auth.uid());

-- Contractor matches policies
DROP POLICY IF EXISTS "Users can view their own matches" ON public.contractor_matches;
CREATE POLICY "Users can view their own matches" ON public.contractor_matches
  FOR SELECT USING (homeowner_id = auth.uid() OR contractor_id = auth.uid());

DROP POLICY IF EXISTS "Homeowners can create matches" ON public.contractor_matches;
