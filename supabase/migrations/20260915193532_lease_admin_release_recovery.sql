ALTER TABLE public.escrow_admin_release_operations
...
CREATE INDEX escrow_admin_release_recovery_due ON public.escrow_admin_release_operations(recovery_after,created_at)
 WHERE state='reserved';

CREATE FUNCTION public.claim_admin_release_recovery() RETURNS SETOF public.escrow_admin_release_operations
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE selected_id uuid;
BEGIN
 SELECT id INTO selected_id FROM public.escrow_admin_release_operations
 WHERE state='reserved'
 ...
 RETURN QUERY UPDATE public.escrow_admin_release_operations
...
CREATE FUNCTION public.finish_admin_release_recovery(p_operation_id uuid,p_token uuid,p_error text DEFAULT NULL)
...
BEGIN
 ...
 UPDATE public.escrow_admin_release_operations SET recovery_token=NULL,recovery_lease_until=NULL,
 ...
 WHERE id=p_operation_id AND recovery_token=p_token AND recovery_lease_until>clock_timestamp();
 RETURN FOUND;
END $$;
REVOKE ALL ON FUNCTION public.claim_admin_release_recovery() FROM PUBLIC,anon,authenticated;
REVOKE ALL ON FUNCTION public.finish_admin_release_recovery(uuid,uuid,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.claim_admin_release_recovery() TO service_role;
GRANT EXECUTE ON FUNCTION public.finish_admin_release_recovery(uuid,uuid,text) TO service_role;
