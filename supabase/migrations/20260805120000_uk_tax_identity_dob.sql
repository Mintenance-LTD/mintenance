-- Migration: UK market readiness (Task 2) — capture DOB + UK tax identity.
-- Date: 2026-08-05
--
-- Additive only — backward-compatible with currently-running code.
-- Applied live to project ukrjudtlvapiajkjbcrd via Supabase MCP on 2026-08-05.
--
-- Sensitive identifiers (DOB, UTR, NINO) are stored ENCRYPTED at rest as an
-- AES-256-GCM EncryptedField envelope (see apps/web/lib/encryption/
-- field-encryption.ts) in jsonb columns, mirroring how totp_secret and other
-- special-category-adjacent data are handled. Never SELECT these into logs;
-- client reads are revoked (service-role + decrypt only).
