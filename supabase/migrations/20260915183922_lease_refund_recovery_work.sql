ALTER TABLE public.escrow_refund_operations
...
CREATE INDEX escrow_refund_recovery_due ON public.escrow_refund_operations(recovery_after,created_at)
 WHERE state IN('reserved','pending','requires_action');

CREATE FUNCTION public.claim_refund_recovery() RETURNS SETOF public.escrow_refund_operations
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE selected_id uuid;
BEGIN
 SELECT id INTO selected_id FROM public.escrow_refund_operations
 WHERE state IN('reserved','pending','requires_action')
 ...
 RETURN QUERY UPDATE public.escrow_refund_operations
...
CREATE FUNCTION public.finish_refund_recovery(p_operation_id uuid,p_token uuid,p_error text DEFAULT NULL)
...
BEGIN
 ...
 UPDATE public.escrow_refund_operations SET recovery_token=NULL,recovery_lease_until=NULL,
 ...
 RETURN FOUND;
END $$;
REVOKE ALL ON FUNCTION public.claim_refund_recovery() FROM PUBLIC,anon,authenticated;
REVOKE ALL ON FUNCTION public.finish_refund_recovery(uuid,uuid,text) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.claim_refund_recovery() TO service_role;
GRANT EXECUTE ON FUNCTION public.finish_refund_recovery(uuid,uuid,text) TO service_role;
