-- R4 of docs/RETENTION_ROADMAP_2026.md — credential moat.
--
-- Per-credential verification (Gas Safe, NICEIC, TrustMark) separate
-- from the existing per-profile `admin_verified` / DBS flow. The
-- PROVIDER ABSTRACTION in CredentialRegisterService.ts can swap from
-- admin-manual to live-API calls once procurement completes, with no
-- schema change.
--
-- Status lifecycle:
--   pending   -> user submitted, awaiting verification
--   verified  -> register/admin confirmed the registration
--   rejected  -> register returned no-match or admin rejected
--   expired   -> verification_expires_at passed; next render treats as needs-renew
...
CREATE TABLE IF NOT EXISTS public.credential_verifications (
  ...
  evidence_url          TEXT        NULL, -- signed Supabase Storage URL for the cert photo
  evidence_path         TEXT        NULL, -- storage_path for regenerating signed URLs
  verified_at           TIMESTAMPTZ NULL,
  verified_by           UUID        NULL REFERENCES auth.users(id),
  expires_at            TIMESTAMPTZ NULL,
  rejected_reason       TEXT        NULL,
  raw_response          JSONB       NULL, -- provider response when API auto-verifies
...
-- Prevent duplicate rows for the same (user, register, registration_number).
...
-- Users may NOT update/delete their own rows once submitted — only admins
-- (service_role) can flip status. Users can always re-submit with a new row.
