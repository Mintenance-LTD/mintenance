-- Token refresh does not create a new Auth session. A refreshed JWT must not
-- resurrect a session established before the account revocation cutoff.
CREATE OR REPLACE FUNCTION public.verified_mobile_session_context(p_user_id uuid, p_session_id uuid, p_issued_at bigint)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
  ...
  WHERE s.id=p_session_id AND s.user_id=p_user_id
    ...
    AND (p.tokens_revoked_at IS NULL OR (
      s.created_at>=p.tokens_revoked_at
      AND to_timestamp(p_issued_at)>=date_trunc('second',p.tokens_revoked_at)
    ))
