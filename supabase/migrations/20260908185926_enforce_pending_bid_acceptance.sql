-- Check status under the bid row lock so a completed withdrawal wins over stale API reads.
...
BEGIN
  ...
  IF v_bid_status IS DISTINCT FROM 'pending' THEN
    RETURN QUERY SELECT FALSE, 'Only pending bids can be accepted'::text, p_bid_id, v_job_status;
    RETURN;
  END IF;
