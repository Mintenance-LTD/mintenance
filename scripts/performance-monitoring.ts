...
class PerformanceMonitor {
  ...
  private checkThresholds(metric: PerformanceMetric) {
    ...
    if (threshold && metric.value > threshold) {
      console.warn(
        `⚠️ Performance threshold exceeded: ${metric.name} took ${metric.value}ms (threshold: ${threshold}ms)`
      );
    }
  }
  ...
  monitorPayment<T>(
    ...
    return paymentFn()
      .then((result) => {
        const duration = Date.now() - startTime;
        this.recordMetric('payment_processing', duration, {
          paymentType,
          success: true,
        });
        return result;
  ...
  monitorAIAnalysis<T>(
    ...
    return analysisFn()
      .then((result) => {
        const duration = Date.now() - startTime;
        this.recordMetric('ai_analysis', duration, {
          analysisType,
          success: true,
        });
        return result;
