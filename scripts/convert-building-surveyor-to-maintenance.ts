...
function extractMaterials(assessment: any): string[] {
  const materials = [];
  const damageType = assessment.damage_type?.toLowerCase();
...
function extractTools(assessment: any): string[] {
  const tools = [];
  const damageType = assessment.damage_type?.toLowerCase();
...
function estimateHours(assessment: any): number {
...
function extractSafetyNotes(assessment: any): string[] {
  const notes = [];
