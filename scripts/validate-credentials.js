...
if (fs.existsSync(appleCredentialsPath)) {
  ...
  if (appleCredentials.includes('your-apple-id@example.com')) {
    validationErrors.push(
      '❌ apple-credentials.env contains placeholder values'
    );
  } else {
...
const googleServiceAccountPath = path.join(
  credentialsDir,
...
if (fs.existsSync(googleServiceAccountPath)) {
  ...
  try {
    const serviceAccount = JSON.parse(
      fs.readFileSync(googleServiceAccountPath, 'utf8')
    );
  ...
  try {
    ...
    if (easConfig.submit?.production) {
      ...
      if (production.ios) {
        ...
        if (ios.appleId?.includes('YOUR_')) {
          validationErrors.push(
            '❌ eas.json contains placeholder values for iOS'
          );
        } else {
      ...
      if (production.android) {
        ...
        if (android.track === 'internal') {
          warnings.push(
            '⚠️ Android track is set to "internal" - change to "production" for store release'
          );
        } else {
...
for (const envVar of requiredEnvVars) {
  if (process.env[envVar]) {
    ...
    warnings.push(
      `⚠️ ${envVar} not set - ensure it's configured in production`
    );
  }
}
