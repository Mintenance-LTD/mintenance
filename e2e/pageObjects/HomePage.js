...
class HomePage {
  ...
  get browseJobsButton() {
    return element(by.id('browse-jobs-button'));
  }
  get quickActionsSection() {
    return element(by.id('quick-actions'));
  }
  get recentJobsSection() {
    return element(by.id('recent-jobs'));
  }
  get notificationBadge() {
    return element(by.id('notification-badge'));
  }
  get searchBar() {
  ...
  // Navigation elements
  get jobsTab() {
    return element(by.id('jobs-tab'));
  }
  get messagesTab() {
    return element(by.id('messages-tab'));
  }
  get notificationsTab() {
    return element(by.id('notifications-tab'));
  }
  get profileTab() {
    return element(by.id('profile-tab'));
  }

  // Actions
  async waitForScreenToLoad() {
    await waitFor(this.welcomeMessage).toBeVisible().withTimeout(10000);
  }
