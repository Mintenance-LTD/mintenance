...
describe('Maintenance App E2E Tests', () => {
  ...
  describe('Job Posting Flow (Homeowner)', () => {
    ...
    it('should create a new job posting', async () => {
      ...
      await element(by.id('job-description-input')).typeText(
        'Leaky kitchen faucet needs professional repair. Water dripping constantly.'
      );
      await element(by.id('job-location-input')).typeText(
    ...
    it('should allow editing job details', async () => {
      ...
      await element(by.id('job-title-input')).typeText(
        'Urgent Kitchen Faucet Repair'
      );
      ...
      await expect(
        element(by.text('Urgent Kitchen Faucet Repair'))
      ).toBeVisible();
    });
  });
  ...
  describe('Job Browsing Flow (Contractor)', () => {
    ...
    it('should view job details and submit bid', async () => {
      ...
      await element(by.id('bid-description-input')).typeText(
        'I can fix this today. 5+ years experience with kitchen plumbing.'
      );
  ...
  describe('Bidding and Acceptance Flow', () => {
    it('should show bid to homeowner and allow acceptance', async () => {
      ...
      await waitFor(element(by.text('Job Assigned')))
        .toBeVisible()
        .withTimeout(5000);
    });
  });
  ...
  describe('Messaging Flow', () => {
    it('should allow messaging between homeowner and contractor', async () => {
      ...
      await expect(
        element(by.text('Hi! When can you start the repair?'))
      ).toBeVisible();
    });
    ...
    it('should show real-time message delivery', async () => {
      ...
      await waitFor(element(by.text('I can start tomorrow at 9 AM')))
        .toBeVisible()
        .withTimeout(15000);
  ...
  describe('Job Completion Flow', () => {
    it('should allow contractor to mark job as completed', async () => {
      ...
      await element(by.id('completion-notes-input')).typeText(
        'Faucet repaired successfully. No more leaks.'
      );
      await element(by.id('confirm-completion')).tap();

      await waitFor(element(by.text('Job Marked as Complete')))
        .toBeVisible()
        .withTimeout(5000);
    });
    ...
    it('should allow homeowner to confirm completion', async () => {
      ...
      await waitFor(element(by.text('Payment Released Successfully!')))
        .toBeVisible()
        .withTimeout(5000);
    });
  });
  ...
  describe('AI Analysis Feature', () => {
    it('should show AI analysis of job photos', async () => {
      ...
      await element(by.id('job-description-input')).typeText(
        'Electrical outlet sparking when plugged in'
      );
      await element(by.id('add-photo-button')).tap();
      await element(by.text('Camera')).tap();

      // Wait for AI analysis
      await waitFor(element(by.id('ai-analysis-section')))
        .toBeVisible()
        .withTimeout(15000);
