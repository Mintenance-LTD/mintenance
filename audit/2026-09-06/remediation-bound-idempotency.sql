...
BEGIN
 SELECT * INTO r FROM public.claim_fenced_idempotency('audit-bound','test',
 ...
 BEGIN
  PERFORM public.claim_fenced_idempotency('audit-bound','test',
  'e395bb8c-efbf-4d06-95f5-55c509de4301',repeat('b',64),60,86400);
  RAISE EXCEPTION 'Changed payload accepted';
 EXCEPTION WHEN invalid_parameter_value THEN NULL; END;
 PERFORM public.complete_fenced_idempotency('audit-bound','test',
 'e395bb8c-efbf-4d06-95f5-55c509de4301',token,'{"receipt":"synthetic"}','{"resource":"synthetic"}');
 SELECT * INTO r FROM public.claim_fenced_idempotency('audit-bound','test',
 'e395bb8c-efbf-4d06-95f5-55c509de4301',repeat('a',64),60,86400);
...
RESET ROLE;
DO $$ BEGIN
 IF has_function_privilege('authenticated','public.claim_fenced_idempotency(text,text,uuid,text,integer,integer)','EXECUTE') OR
    has_function_privilege('anon','public.claim_fenced_idempotency(text,text,uuid,text,integer,integer)','EXECUTE') THEN
