...
try:
    ...
    def reserve(index):
        try:
            return sql(f"BEGIN; SELECT id FROM public.reserve_admin_escrow_refund('{administrator}','{job}','{escrow}','refund-{job}-{index}',10000,'Synthetic'); SELECT pg_sleep(0.4); COMMIT;")
    ...
    assert sql(f"SELECT count(*) FROM public.notifications WHERE metadata->>'refundOperationId'='{op}'") == '2'
    assert sql(f"SELECT count(*) FROM public.audit_logs WHERE new_values->>'operation_id'='{op}'") == '1'
    print('PASS: concurrent admin refunds reserve once; concurrent settlement deducts principal and emits notifications/audit once')
finally:
