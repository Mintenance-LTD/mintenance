...
try:
    ...
    assert sql("SELECT count(*) FROM public.escrow_admin_release_operations WHERE state='reserved' AND created_at<now()-interval '2 minutes' AND recovery_after<=now() AND (recovery_lease_until IS NULL OR recovery_lease_until<=now())") == '0', 'Other due audit work exists; do not claim it'
    op=sql(f"SELECT id FROM public.reserve_admin_escrow_release('{administrator}','{escrow}','Synthetic release',0.12)")
    sql(f"UPDATE public.escrow_admin_release_operations SET created_at=now()-interval '5 minutes' WHERE id='{op}'")
    def claim(_):
        return sql("BEGIN; SET LOCAL ROLE service_role; SELECT recovery_token FROM public.claim_admin_release_recovery(); SELECT pg_sleep(0.4); COMMIT;")
    ...
    old_token=winners[0]
    sql(f"UPDATE public.escrow_admin_release_operations SET recovery_lease_until=now()-interval '1 second' WHERE id='{op}'")
    new_token=sql("SET ROLE service_role; SELECT recovery_token FROM public.claim_admin_release_recovery()")
    assert new_token and old_token != new_token, 'Expired claim was not replaced'
    assert sql(f"SET ROLE service_role; SELECT public.finish_admin_release_recovery('{op}','{old_token}',NULL)")=='f'
    assert sql(f"SET ROLE service_role; SELECT public.finish_admin_release_recovery('{op}','{new_token}',NULL)")=='t'
