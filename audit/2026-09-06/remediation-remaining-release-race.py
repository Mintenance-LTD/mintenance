...
try:
    ...
    held_transaction(
        f"SELECT id FROM public.reserve_escrow_refund('{owner}','{job}','{escrow}','refund-{job}',10000,'Synthetic'); SELECT id FROM public.record_escrow_refund_outcome((SELECT id FROM public.escrow_refund_operations WHERE escrow_id='{escrow}'),'re_synthetic_{job}','succeeded'); UPDATE public.jobs SET status='assigned' WHERE id='{job}'; UPDATE public.jobs SET status='in_progress' WHERE id='{job}'; UPDATE public.jobs SET status='completed' WHERE id='{job}'; INSERT INTO public.job_photos_metadata(job_id,photo_url,photo_type,verified) VALUES('{job}','https://example.invalid/synthetic-after','after',true); SELECT public.approve_job_completion('{job}','{owner}',NULL,'{escrow}',NULL,false,true)",
