...
try:
    ...
    held_transaction(
        f"SELECT id FROM public.reserve_escrow_refund('{owner}','{job}','{escrow}','refund-{job}',10000,'Synthetic'); SELECT id FROM public.record_escrow_refund_outcome((SELECT id FROM public.escrow_refund_operations WHERE escrow_id='{escrow}'),'re_synthetic_{job}','succeeded'); UPDATE public.jobs SET status='assigned' WHERE id='{job}'; UPDATE public.jobs SET status='in_progress' WHERE id='{job}'; UPDATE public.jobs SET status='completed' WHERE id='{job}'",
        f"SELECT remaining_minor FROM public.claim_escrow_release('{escrow}','manual_release',gen_random_uuid())",
        expected_result='40000')
    sql(f"UPDATE public.escrow_transactions SET status='held' WHERE id='{escrow}'")
    held_transaction(
        f"SELECT remaining_minor FROM public.claim_escrow_release('{escrow}','manual_release',gen_random_uuid())",
        f"SELECT id FROM public.reserve_escrow_refund('{owner}','{job}','{escrow}','after-claim-{job}',1000,'Synthetic')",
        expected_error='Escrow cannot be claimed for refund')
    sql(f"UPDATE public.escrow_transactions SET status='held' WHERE id='{escrow}'")
    held_transaction(
        f"SELECT remaining_minor FROM public.claim_escrow_release('{escrow}','manual_release',gen_random_uuid())",
        f"SELECT remaining_minor FROM public.claim_escrow_release('{escrow}','auto_release',gen_random_uuid())",
        expected_result='')
    print('PASS: waiting release reads committed partial-refund balance; release excludes refunds and a second release')

finally:
    sql(f"DELETE FROM public.contractor_payout_credit_events WHERE job_id='{job}'; DELETE FROM public.contractor_payout_balances WHERE contractor_id='{contractor}'; DELETE FROM public.escrow_refund_operations WHERE escrow_id IN(SELECT id FROM public.escrow_transactions WHERE job_id='{job}'); DELETE FROM public.escrow_refund_balances WHERE escrow_id IN(SELECT id FROM public.escrow_transactions WHERE job_id='{job}'); DELETE FROM public.payment_funding_reservations WHERE job_id='{job}'; DELETE FROM public.escrow_transactions WHERE job_id='{job}'; DELETE FROM public.contracts WHERE id='{contract}'; DELETE FROM public.bids WHERE id='{bid}'; DELETE FROM public.jobs WHERE id='{job}'; DELETE FROM auth.users WHERE id IN ('{owner}','{contractor}');")
