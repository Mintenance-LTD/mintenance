...
const args=process.argv[2]==='dev'?['dev','--webpack','-p','3017']:['build','--webpack'];
const command=process.argv[2]==='dev'?[path.resolve('audit/2026-09-06/dev-server.cjs')]:[path.resolve('node_modules/next/dist/bin/next'),...args];
const child=spawn(process.execPath,command,{cwd:path.resolve('apps/web'),env,stdio:'inherit'});
