...
user = None
try:
    email = 'audit-reauth-' + str(uuid.uuid4()) + '@example.invalid'
    ...
    assert code in (200,201), 'Synthetic account creation failed'
    user = str(uuid.UUID((created.get('user') or created)['id']))
    credentials = {'email':email,'password':password}
    code, existing = api('/auth/v1/token?grant_type=password','POST',credentials)
    assert code == 200, 'Existing session creation failed'
    code, temporary = api('/auth/v1/token?grant_type=password','POST',credentials)
    assert code == 200 and temporary['user']['id'] == user, 'Reauthentication failed'
    code, _ = api('/auth/v1/logout?scope=local','POST',token=temporary['access_token'])
    assert code in (200,204), 'Temporary session removal failed'
    code, _ = api('/auth/v1/token?grant_type=refresh_token','POST',{'refresh_token':temporary['refresh_token']})
    assert code == 400, 'Temporary session still refreshes'
    code, _ = api('/auth/v1/token?grant_type=refresh_token','POST',{'refresh_token':existing['refresh_token']})
    assert code == 200, 'Unrelated existing session was signed out'
    print('PASS: local reauthentication identifies the account; local-scope logout removes only the temporary session')
finally:
    if user:
