...
for (const dir of ['.','apps/web','apps/mobile']) for (const name of fs.readdirSync(dir)) {
...
env.NODE_ENV="test";
const child=spawn(process.execPath,[path.resolve("node_modules/jest/bin/jest.js"),"--config","apps/mobile/jest.config.js","--runInBand",...process.argv.slice(2)],{cwd:process.cwd(),env,stdio:"inherit"});
