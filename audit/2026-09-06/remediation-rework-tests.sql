...
CREATE TRIGGER audit_rework_notification_fail BEFORE INSERT ON public.notifications FOR EACH ROW EXECUTE FUNCTION public.audit_rework_fail();
DO $$ BEGIN
 BEGIN
  PERFORM public.request_job_rework('fa060906-0000-4000-8000-000000000010','fa060906-0000-4000-8000-000000000001','synthetic-rework-key','Fix the finish');
  RAISE EXCEPTION 'Notification fault was not reached';
 EXCEPTION WHEN SQLSTATE 'ZX001' THEN NULL; END;
 IF (SELECT status FROM public.jobs WHERE id='fa060906-0000-4000-8000-000000000010')<>'completed'
  OR EXISTS(SELECT FROM public.job_rework_requests WHERE job_id='fa060906-0000-4000-8000-000000000010') THEN
  RAISE EXCEPTION 'Notification failure committed incomplete rework'; END IF;
END $$;
DROP TRIGGER audit_rework_notification_fail ON public.notifications;
...
DO $$ BEGIN
  ...
  IF EXISTS (SELECT 1 FROM public.escrow_transactions WHERE job_id='fa060906-0000-4000-8000-000000000010' AND (status<>'held' OR homeowner_approval OR auto_release_date IS NOT NULL OR auto_approval_date IS NOT NULL OR cooling_off_ends_at IS NOT NULL)) THEN RAISE EXCEPTION 'Approval not reset'; END IF;
  IF (SELECT count(*) FROM public.notifications WHERE metadata->>'jobId'='fa060906-0000-4000-8000-000000000010' AND type='changes_requested')<>1 THEN RAISE EXCEPTION 'Rework notification missing or duplicated'; END IF;
  BEGIN
