"""Real photo-verification/rework/approval lock races, on the disposable audit DB only."""
...
cases = [
 ('verify','rework',False,'in_progress/held/false/none'),
 ('rework','verify',True,'in_progress/held/false/none'),
 ('verify','approve',False,'completed/held/true/none'),
 ('approve','verify',True,'completed/held/true/none'),
]
            ...
            if not blocked:
                ...
                try:
                    ...
                    assert any(message in str(error) for message in [
                        'The completion changed', 'already approved', 'not available for approval', 'Completion changed'])
